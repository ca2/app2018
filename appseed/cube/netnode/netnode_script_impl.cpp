...
netnodeScriptImpl::netnodeScriptImpl(netnodeScript * pscript) :
   ca(pscript->get_app()),
...
netnodeScriptImpl::~netnodeScriptImpl()
{
}

void netnodeScriptImpl::run()
{
/*   print("<html>\n");
   print("<head>\n");
   print("</head>\n");
...
void netnodeScriptImpl::script_start()
{
   m_pinstanceMain->m_str2aScript.add_new();
   m_pinstanceMain->ob_start();
}

void netnodeScriptImpl::script_step()
{
   string str;
...
void netnodeScriptImpl::script_end()
{
   string str;
   m_pinstanceMain->ob_get_clean().to_string(str);
   m_pinstanceMain->m_str2aScript.last_element().add(str);
...
gen::property & netnodeScriptImpl::gprop(const char * pszKey)
{
...
void netnodeScriptImpl::echo(const char * lpcsz)
{
   print(lpcsz);
}

void netnodeScriptImpl::print_r(var var)
{
   print(var.to_r_string());
}

void netnodeScriptImpl::print(const char * lpcsz)
{
   string str(lpcsz);
...
void netnodeScriptImpl::raw_print(const char * lpcsz)
{
   string str(lpcsz);
...
void netnodeScriptImpl::ds_print(const char * lpcsz)
{
   raw_print(lpcsz);
}

void netnodeScriptImpl::printf(const char * lpcsz, ...)
{
...
void netnodeScriptImpl::printstr(const char * lpcsz, ...)
{
   string strIndex;
   string str = lpcsz;
   va_list arguments;       
...
void netnodeScriptImpl::print_url(const char * pszUrl)
{
   print(a_go1(pszUrl));
}


bool netnodeScriptImpl::read_file(const char * lpcsz, array_ptr_alloc < int_array > * prangea)
{
   return m_pnetnodesocket->read_file(lpcsz, prangea);
}

string netnodeScriptImpl::real_path(const char * psz)
{
...
string netnodeScriptImpl::real_path(const char * pszBase, const char * psz)
{
   return get_manager()->real_path(pszBase, psz);
}

string netnodeScriptImpl::system_path(const char * psz)
{
   return real_path(psz);
}

var netnodeScriptImpl::include(const char * lpcsz)
{
   ...
   netnodeScriptManager * pmanager = papp->get_script_manager();
   string strHead;
   http::memory_file memfile(get_app());
   netnodeScriptInstance * pinstance = pmanager->get_output_internal(this, strInclude);
   if(pinstance == NULL)
      return var();
   else
   {
...
void netnodeScriptImpl::launch(const char * lpcsz)
{
   netnode::application * papp = &Application;
...
gen::property & netnodeScriptImpl::get(const char * pszKey)
{
   return m_pnetnodesocket->request().form().get()[pszKey];
}
gen::property & netnodeScriptImpl::post(const char * pszKey)
{
   return m_pnetnodesocket->request().form().post()[pszKey];
}
http::cookie & netnodeScriptImpl::cookie(const char * pszKey)
{
   return m_pnetnodesocket->response().cookies().cookie(pszKey);
}
gen::property & netnodeScriptImpl::request(const char * pszKey)
{
   return m_pnetnodesocket->request().form().request()[pszKey];
}
gen::property & netnodeScriptImpl::session(const char * pszKey)
{
   m_pinstanceMain->session_ensure();
   return (*m_pinstanceMain->m_ppropertysetSession)[pszKey];
}
gen::relation_set & netnodeScriptImpl::geta()
{
   return m_pnetnodesocket->request().form().get();
}
gen::relation_set & netnodeScriptImpl::posta()
{
   return m_pnetnodesocket->request().form().post();
}
http::cookies & netnodeScriptImpl::cookies()
{
   return m_pnetnodesocket->request().cookies();
}
gen::relation_set & netnodeScriptImpl::requesta()
{
   return m_pnetnodesocket->request().form().request();
}
gen::property_set & netnodeScriptImpl::sessiona()
{
   m_pinstanceMain->session_ensure();
   return *m_pinstanceMain->m_ppropertysetSession;
}

gen::property_set & netnodeScriptImpl::inattra()
{
   return m_pnetnodesocket->inattrs();
}

string netnodeScriptImpl::this_url()
{
...
string netnodeScriptImpl::session_id(const char * pszId)
{
...
void netnodeScriptImpl::edgify(int iEdge, ::userbase::document_template * ptemplate, bool bMakeVisible)
{
   //System.edgify(iEdge, ptemplate, bMakeVisible);
}

void netnodeScriptImpl::edgify_app(int iEdge, ::ca::type_info prtcApp, bool bMakeVisible)
{
   //System.edgify_app(iEdge, prtcApp, bMakeVisible);
}

void netnodeScriptImpl::hand_root()
{
   ...
   dprint("<h3>netnodeScriptImpl::hand_root()</h3>");
   until_here();
   ...
   else
   {
      ...
      {
         m_pinstanceMain->m_strCa2OpenWebSite         = "https://fontopus.com/";
         m_pinstanceMain->m_strImageWebSite           = "http://image." + gen::str::has_char(straDomainSuffix.implode("."), NULL, ".") + "veriterse.net/image/";
         m_pinstanceMain->m_strCssWebSite             = "http://css." + gen::str::has_char(straDomainSuffix.implode("."), NULL, ".") + "veriterse.net/css/";
         m_pinstanceMain->m_strFileWebSite            = "http://file." + gen::str::has_char(straDomainSuffix.implode("."), NULL, ".") + "veriterse.net/file/";
         m_pinstanceMain->m_strCa2VFallbackWebSite    = "http://ca2v." + gen::str::has_char(straDomainSuffix.implode("."), NULL, ".") + "veriterse.net/";
      }
   }
   ...
   else if(strScript == "/sec" && (gen::str::ends(inattr("http_host"), ".fontopus.com") || inattr("http_host") == "fontopus.com"
      || get("action") == "logout"))
   {
      hand_sec();
   }
   ...
   else
   {
      ...
      if(strInclude.has_char())
      {
         if(ca2_fetch_mode() != 1)
         {
            dprint("<h4>netnodeScriptImpl::_on_before_include() root</h4>");
            until_here();
            ...
            {
               dprint("<h4>netnodeScriptImpl::_on_before_include() subdomain</h4>");
               until_here();
               include(eval("site/$param_root/subdomain/$param_subdomain/secure/_on_before_include.ds"));
            }
         }
         dprint("<h3>netnodeScriptImpl::hand_root() Include</h3>");
         until_here();
...
void netnodeScriptImpl::hand_sec()
{
   prof_enter("netnodeScriptImpl::hand_sec()");
...
//      dprint_r($_POST);
   ...
   prof_leave("netnodeScriptImpl::hand_sec()");
}

void netnodeScriptImpl::hand_err()
{
...
void netnodeScriptImpl::hand_karaoke()
{
}

void netnodeScriptImpl::hand_cifras()
{
}

void netnodeScriptImpl::hand_letras()
{
}


void netnodeScriptImpl::ui_nav_printOut5(stringa & straLink, stringa & straTitle)
{
...
void netnodeScriptImpl::ui_nav_printOut3(stringa & straLink, stringa & straTitle)
{
   int iCount = straTitle.get_size() - 1;
...
void netnodeScriptImpl::ui_nav_printOut2(stringa & straLink, stringa & straTitle)
{
   int iCount = straTitle.get_size() - 1;
   int i;
   for(i = 0; i < iCount; i++)
...
void netnodeScriptImpl::ui_nav_printOut1(stringa & straLink, stringa & straTitle)
{
...
bool netnodeScriptImpl::script_right()
{
   return false;
}
   ...
   void netnodeScriptImpl::dprint(const char * psz)
   {
   ...
   void netnodeScriptImpl::dprint_r(var var)
   {
      if(m_pinstanceMain->m_iDebug > 0)
      {
         print_r(var);
   ...
   void netnodeScriptImpl::dinit()
   {
      ...
      {
         m_pprofiler = new netnode::profiler();
      }
   }

   __int64 netnodeScriptImpl::get_first_time()
   {
      return m_pnetnodesocket->m_iFirstTime;
   }

   void netnodeScriptImpl::prof_enter(const char * pszName)
   {
   ...
   void netnodeScriptImpl::prof_leave(const char * pszName)
   {
      if(m_pinstanceMain->m_pprofiler != NULL)
      {
   ...
   void netnodeScriptImpl::prof_display()
   {
      if(m_pinstanceMain->m_pprofiler != NULL)
      {
   ...
   void netnodeScriptImpl::until_here()
   {
      __int64 iTime;
      {
         __int64 count;
   ...
   class netnode::profiler * netnodeScriptImpl::profiler()
   {
      return m_pinstanceMain->m_pprofiler;
   }
   
   void netnodeScriptImpl::dfinish()
   {
      //global $g_debug;
      //global $g_profiler;
   }
   
   /*string netnodeScriptImpl::defer_explain_mysqli(&$mysqli, $sql)
   {
      global $g_debug;
...
void netnodeScriptImpl::root_init()
{
   dprint("<br>INIT PATH");
...
/*   global $g_listline;
   global $userdir;
   global $enable_userdir;
   global $g_langdir_modifier;
   global $"g_langdir_modifier_ex";
   global $g_override_userdir;
   global $g_subdomain_modifier;
   global $g_subdomain_modifier_ex;
   global $g_userdir_modifier;
   global $"param_locale";
   global $webalbum_dir;
   global $param_root;
   global $param_subdomain;
   global $site_modifier;
   global $g_base_url_path;
   global $g_base_url;
   global $"param_locale"_ex;
   global $g_css_modifier_prefix;*/
   
   /*if(gstra(use_style_get_accepta).get_size() > 0)
   {
      include "site/karaoke/library/music/database/_lib.ds";
   }*/
   

   gstr("g_css_modifier_prefix") = use_style_calc_initial_css_modifier_prefix();
   
   //dprint("g_css_modifier_prefix='$g_css_modifier_prefix'");

   //$"g_langdir_modifier_ex" = base_array();
   ...
   /*$"g_langdir_modifier_ex" = base_array();
   $g_subdomain_modifier_ex = base_array();
   */
   if(gprop("enable_userdir"))
   {
...
/*      if(is_array($"param_locale"_ex))
      {
         foreach($"param_locale"_ex as $ikey=>$lang)
         {
            $style = $"param_style"_ex[$ikey];
            $"g_langdir_modifier_ex"[] = "$lang/$style/";
         }
      }*/
...
*/
   ...
   //dprint("systeminit(param_sub)=$param_subdomain");
   ...
   /*dprint("systeminit(g_subdomain_modifier)=$g_subdomain_modifier<br />");
   dprint("systeminit(g_subdomain_modifier_ex):<br />");
   dprint_r($g_subdomain_modifier_ex);
   
   
   if(strlen($g_base_url) == 0)
   {
      $g_base_url = "/";
   }
   else if(substr($g_base_url, -1, 1) != "/")
   {
      $g_base_url .= "/";
   }
   
   if(strlen($g_base_url_path) == 0)
   {
      $g_base_url_path = "/";
   }
   ...
   */
   ...
   string netnodeScriptImpl::ui_redir(const char * pszLang, const char * pszStyle, const char * pszExUri, const char * pszRoot)
{
   CSingleLock sl(&m_pmanager->m_csUiRedir, TRUE);
...
void netnodeScriptImpl::ui_redir_add(const char * pszRoot, const char * pszLang, const char * pszStyle, const char * pszTarget)
{
   CSingleLock sl(&m_pmanager->m_csUiRedir, TRUE);
   return uiredir().add(pszRoot, pszLang, pszStyle, pszTarget);
}

string netnodeScriptImpl::a_go1(var var)
{
   ...
   if(!set["lang"].is_set())
   {
      strUrl = System.url().set(strUrl, "lang", gprop("param_locale"));
   }
   if(!set["styl"].is_set())
   {
      strUrl = System.url().set(strUrl, "styl", gprop("param_style"));
   }
   return strUrl;
}

string netnodeScriptImpl::ui_exuri()
{
   return "";
}

class ui_redir & netnodeScriptImpl::uiredir()
{
   return m_pscript->m_pmanager->m_uiredir;
}

string netnodeScriptImpl::sys_get_include_path(const char * pszExt, const char * pszType, const char * pszDoc, stringa * pstraAccept)
{
...
string netnodeScriptImpl::named_sys_get_include_path(const char * pszSystemPath, const char * pszExt, const char * pszType, const char * pszDoc, stringa * pstraAccept)
{
   ...
   netnode::profiler::scope scopeProfiler(profiler(), "netnodeScriptImpl::sys_get_include_path", true);
...
string netnodeScriptImpl::sys_get_base_path(const char * pszExt, const char * pszType, const char * pszDoc, const char *pszBase, stringa * pstraAccept)
{
   if(strlen(gprop("param_site")) > 0)
   {
...
string netnodeScriptImpl::named_sys_get_base_path(const char * pszSystemPath, const char * pszExt, const char * pszType, const char * pszDoc, const char *pszBase, stringa * pstraAccept)
{
...
string netnodeScriptImpl::sys_get_subdomain_path(const char * pszExt, const char * pszType, const char * pszDoc, const char *pszBase, const char * pszSubdomain, stringa * pstraAccept)
{
   if(strlen(gprop("param_site")) > 0)
   {
...
string netnodeScriptImpl::named_sys_get_subdomain_path(const char * pszSystemPath, const char * pszExt, const char * pszType, const char * pszDoc, const char * pszBase, const char * pszSubdomain, stringa * pstraAccept)
{
   gen::property & propLangDirModifierEx = gprop("g_langdir_modifier_ex");
...
/*void netnodeScriptImpl::use_style_get_acceptmap(string_to_string_map & strmap)
   {
   ...
   string netnodeScriptImpl::use_style_get_default()
   {
      if(gstr("param_style") == "veriwell" || gstr("param_root") == "veriwell")
      {
         return "2008-04-02";
   ...
   string netnodeScriptImpl::use_style_calc_initial_css_modifier_prefix()
   {
...
*/
   /*string netnodeScriptImpl::use_style_set_style($id)
   {
      global $musicdb;
      global $secureuserid;
   ...
   string netnodeScriptImpl::use_style_get_path_prefix($id)
   {
      $accepta = use_style_get_accepta();
      if(count($accepta) == 0)
         return '';
...
void netnodeScriptImpl::menuleft_printVert001(ui_menu * pmenu, int iPanelIndex)
{
...
/*
string netnodeScriptImpl::menu_printOutNav1($linka, $titlea)
{
...
/*
string netnodeScriptImpl::menu_printOutNav2($linka, $titlea)
{
   require_once "left.inc.ds";
   ?>
...
gen::property & netnodeScriptImpl::inheader(const char * psz)
{
   return m_pnetnodesocket->inheader(psz);
}

gen::property & netnodeScriptImpl::outheader(const char * psz)
{
   return m_pnetnodesocket->outheader(psz);
}


gen::property & netnodeScriptImpl::inattr(const char * psz)
{
   return m_pnetnodesocket->inattr(psz);
}

gen::property & netnodeScriptImpl::outattr(const char * psz)
{
   return m_pnetnodesocket->outattr(psz);
}

netnodeSocket & netnodeScriptImpl::netnode()
{
   return *m_pnetnodesocket;
}

gen::property & netnodeScriptImpl::request_uri()
{
   return netnode().request().attr("request_uri");
}


string netnodeScriptImpl::langstr(const char * pszKey)
{
   return gprop("langstr").propset()[pszKey].get_string();
}

string netnodeScriptImpl::langstr2(const char * pszKey)
{
   return gprop("langstr").propset()[pszKey].get_string();
}

void netnodeScriptImpl::print_langstr(const char * pszKey)
{
   print(langstr(pszKey));
}

void netnodeScriptImpl::set_langstr(const char * pszKey, const char * pszValue)
{
   gprop("langstr").propset()[pszKey].get_value().set_string(pszValue);
}

string netnodeScriptImpl::current_url()
{
   return inattr("http_protocol") + "://" + inattr("http_host") + inattr("request_uri");
}


string netnodeScriptImpl::urlencode(const char * psz)
{
   return System.url().url_encode(psz);
}

string netnodeScriptImpl::urldecode(const char * psz)
{
   return System.url().url_decode(psz);
}

string netnodeScriptImpl::htmlspecialchars(const char * psz)
{
   return System.html().special_chars(psz);
}

string netnodeScriptImpl::htmlentities(const char * psz)
{
   return System.html().entities(psz);
}

netnodeScriptManager * netnodeScriptImpl::get_manager()
{
   return m_pmanager;
}


void netnodeScriptImpl::load_stringtable(const char * pszLoad)
{
...
/*   $lang = sys_get_base_path('ds', '', $load_stringtable, 'lang/');
   ...
   string netnodeScriptImpl::eval(const char * pszText)
   {
      string str(pszText);
      int iStart = 0;
      bool bBracket;
...
//   prof_leave('load string table');
   ...
   fontopus_database & netnodeScriptImpl::musicdb_se()
   {
   ...
   fontopus_database & netnodeScriptImpl::admindb()
   {
      if(m_pinstanceMain->m_padmindb == NULL)
      {
         m_pinstanceMain->m_padmindb = new fontopus_database(get_app());
   ...
   fontopus_database & netnodeScriptImpl::netnodedb()
   {
      if(m_pinstanceMain->m_pnetnodedb == NULL)
      {
         m_pinstanceMain->m_pnetnodedb = new fontopus_database(get_app());
   ...
   secure & netnodeScriptImpl::secure()
   {
      if(m_pinstanceMain->m_psecure == NULL)
      {
         m_pinstanceMain->m_psecure = new class secure();
      }
      return *m_pinstanceMain->m_psecure;
   }

   netnode::music & netnodeScriptImpl::music()
   {
      if(m_pinstanceMain->m_pmusic == NULL)
      {
         if(m_pinstanceMain->m_pmusic == NULL)
         {
            m_pinstanceMain->m_pmusic = new netnode::music();
         }
      }
      return *m_pinstanceMain->m_pmusic;
   }
   cynce_database & netnodeScriptImpl::cyncedb()
   {
   ...
   cynce & netnodeScriptImpl::cynce()
   {
      if(m_pinstanceMain->m_pcynce == NULL)
      {
         m_pinstanceMain->m_pcynce = new class cynce();
      }
      return *m_pinstanceMain->m_pcynce;
   }

   netnode::user & netnodeScriptImpl::user()
   {
      if(m_pinstanceMain->m_pui == NULL)
      {
         m_pinstanceMain->m_pui = new class netnode::user();
      }
      return *m_pinstanceMain->m_pui;
   }

   netnode::wb & netnodeScriptImpl::wb()
   {
      if(m_pinstanceMain->m_pwb == NULL)
      {
         m_pinstanceMain->m_pwb = new class netnode::wb();
      }
      return *m_pinstanceMain->m_pwb;
   }

   void netnodeScriptImpl::keep(ptr * pptr)
   {
   ...
   void netnodeScriptImpl::unkeep(ptr * pptr)
   {
      m_pinstanceMain->m_ptra.remove(pptr);
   }

   int netnodeScriptImpl::exit(int iExitCode)
   {
      throw exit_exception(iExitCode);
   }


   bool netnodeScriptImpl::session_open(const char * pszSavePath, const char * pszSessionName) 
   {
   ...
   bool netnodeScriptImpl::session_close() 
   {
      return true;
   }

   bool netnodeScriptImpl::session_read(const char * pszId, gen::property_set & set) 
   {
      if(get("fun_session").is_set() && get("fun_session") == 0)
         return false;
      //return netnodedb().get_session(pszId, set);
      //return musicdb().get_session(pszId, set);
      return get_manager()->get_session(pszId, set);
   }

   bool netnodeScriptImpl::session_write(const char * pszId, gen::property_set & set) 
   {
      if(get("fun_session").is_set() && get("fun_session") == 0)
         return false;
      //return netnodedb().set_session(pszId, set);
      //return musicdb().set_session(pszId, set);
      return get_manager()->set_session(pszId, set);
   }

   bool netnodeScriptImpl::session_destroy(const char * pszId) 
   {
    ...
    *********************************************/
   bool netnodeScriptImpl::session_gc(int maxlifetime) 
   {
     return true;
   }


   bool netnodeScriptImpl::main_initialize()
   {
   ...
   bool netnodeScriptImpl::main_finalize()
   {
      if(this != m_pinstanceMain)
         throw 0;
      ...
      while(countSafe > 0)
      {
         ...
         try
         {
            if(m_ptra.last_element()->m_bUnkeep && m_ptra.last_element()->::ca::ca::is_set(::ca::ca::flag_auto_clean))
            {
   ...
   void netnodeScriptImpl::session_ensure()
   {
      if(get("fun_session").is_set() && get("fun_session") == 0)
         return;
...
string netnodeScriptImpl::uri_set_var(const char * pszQuery, const char * pszKey, var var)
{
   return System.url().set(pszQuery, pszKey, var);
}

string netnodeScriptImpl::get_current_url()
{
...
int netnodeScriptImpl::ca2_fetch_mode(int iMode)
{
   if(iMode >= 0)
   {
...
string netnodeScriptImpl::set_cookie(
      const char * pszKey,
      var varValue,
      int iExpire,
      const char * pszPath, 
...
void netnodeScriptImpl::use_style_get_accepta(gen::property_set & set)
{
...
string netnodeScriptImpl::use_style_calc_initial_css_modifier_prefix()
{
   ...
   if(!isset(get("secure")) || (bool)get("secure"))
   {
...
void netnodeScriptImpl::use_style_set_style(const char * pszId)
{
...
string netnodeScriptImpl::use_style_get_path_prefix(const char * pszId)
{
   gen::property_set accepta;
   use_style_get_accepta(accepta);
   if(accepta.m_propertya.get_count() == 0)
...
string  netnodeScriptImpl::use_style_get1()
{
   ...
   checkimage & netnodeScriptImpl::checkimage()
   {
      if(m_pcheckimage == NULL)
         m_pcheckimage = new class checkimage();
      return *m_pcheckimage;
   }

void netnodeScriptImpl::header(const char * pszHeader)
{
...
void netnodeScriptImpl::simple_file_server(const char * psz, const char * pszRelative)
{
   m_pnetnodesocket->simple_file_server(psz, pszRelative);
}

bool netnodeScriptImpl::isset(var & var)
{
   return var.is_set();
}

bool netnodeScriptImpl::isset(gen::property & prop)
{
   return prop.is_set();
}

bool netnodeScriptImpl::is_null(var & var)
{
   return var.is_null();
}

bool netnodeScriptImpl::is_null(gen::property & prop)
{
   return prop.is_null();
}

string netnodeScriptImpl::pstr(const char * pszTopic, gen::property_set & set, const char * pszLocale, const char * pszStyle)
{
...
string netnodeScriptImpl::pstr(const char * pszTopic, const char * pszLocale, const char * pszStyle)
{
   CSingleLock sl(&m_pmanager->m_csPersistentStr, TRUE);
...
void netnodeScriptImpl::pstr_set(const char * pszTopic, const char * pszLocale, const char * pszStyle, const char * psz)
{
   CSingleLock sl(&m_pmanager->m_csPersistentStr, TRUE);
...
string netnodeScriptImpl::ui_url(const char * pszTopic, const char * pszStyle)
{
   return ui_redir(NULL, pszStyle, pszTopic);
}

void netnodeScriptImpl::cla_sel1(const char * pszSection, const char * pszItem)
{
...
string netnodeScriptImpl::ca2_get_build()
{
   string strPath;
...
string netnodeScriptImpl::ca2_get_subversion_revision()
{
   string strUrl;
...
string netnodeScriptImpl::ca2_format_build()
{
...
string netnodeScriptImpl::datetime_get_month_str(int idx, const char * pszLocale, const char * pszStyle)
{
   string str;
...
string netnodeScriptImpl::datetime_get_month_short_str(int idx, const char * pszLocale, const char * pszStyle)
{
   string str;
...
string netnodeScriptImpl::datetime_get_weekday_short_str(int idx, const char * pszLocale, const char * pszStyle)
{
   string str;
...
string netnodeScriptImpl::datetime_get_weekday_str(int idx, const char * pszLocale, const char * pszStyle)
{
   string str;
...
//string netnodeScriptImpl::datetime_get_month_day_count(int year, int month)


//}
/*
string netnodeScriptImpl::datetime_getGMTDateTime()
{
   return datetime_formatDateTime(datetime_getTimeStamp());
}
*/
/*
string netnodeScriptImpl::datetime_formatDateTime($timestamp)
{
   return gmdate("Y-m-d H:i:s", $timestamp);
}
*/

/*int64_t netnodeScriptImpl::datetime_get_timestamp()
{
   return time(NULL);
}*/

/*
string netnodeScriptImpl::datetime_InternationalToUnixMillis($date)
{
...
/*string netnodeScriptImpl::datetime_getToday()
{
   return gmdate("Y-m-d 00:00:00", time());
}

string netnodeScriptImpl::datetime_strtotime($str, $time = FALSE)
{
   if($time === FALSE)
      $time = time();
...
int64_t netnodeScriptImpl::strtotime(const char * psz, time_t time)
{
   int iPathCount;
...
int64_t netnodeScriptImpl::strtotime(const char * psz)
{
   int iPathCount;
   return System.datetime().strtotime(NULL, psz, 0, iPathCount);
}

int64_t netnodeScriptImpl::str_context_strtotime(const char * psz, time_t time)
{
   int iPathCount;
...
int64_t netnodeScriptImpl::str_context_strtotime(const char * psz)
{
   int iPathCount;
   return System.datetime().strtotime(str_context(), psz, 0, iPathCount);
}

string netnodeScriptImpl::strftime(const char * psz, time_t timeParam)
{
   string str;
...
string netnodeScriptImpl::strftime(const char * psz)
{
   string str;
   class time time;
   time = time::get_current_time();
...
var netnodeScriptImpl::rand(var start, var end)
{
   return System.math().RandRange(start.get_integer(), start.get_integer());
}


void netnodeScriptImpl::echo_spa(const char * psz, const char * pszBuild)
{
   print(spa_url(psz, pszBuild));
}


string netnodeScriptImpl::spa_url(const char * psz, const char * pszBuild)
{
...
void netnodeScriptImpl::hand_krftimg()
{
   var size;
   var bk;
...
void netnodeScriptImpl::mbimage_outputImage(
   var text, 
   var size, 
   var bk)
{
...
void netnodeScriptImpl::mbimage_rectImage(var w, var h)
{
   header("Cache-control: max-age=1, no-store, no-cache, must-revalidate");
   header("Content-type: image/png");
...
COLORREF netnodeScriptImpl::color(var c)
{
   if(c == "w")
   {
   ...
   {
      TRACE("netnodeScriptImpl::color unknwon color code");
   }
   return 0;
}

void netnodeScriptImpl::image_png(::ca::bitmap_sp pbitmap)
{
   FIMEMORY * pfm1 = FreeImage_OpenMemory();
   FIBITMAP * pfi7 = System.imaging().HBITMAPtoFI(pbitmap);
   FIBITMAP * pfi8 = FreeImage_ConvertTo24Bits(pfi7);
...
void netnodeScriptImpl::hand_extension()
{
   string strFile;
   string strUri = gprop("request");
...
void netnodeScriptImpl::ensure_ssl()
{
   if(!m_pinstanceMain->m_pnetnodesocket->IsSSL())
   {
...
bool netnodeScriptImpl::is_common_dir(const char * psz)
{
   string str(psz);
   if(str == "css")
      return true;
...
string netnodeScriptImpl::locale_from_request_v1(const char * psz)
{
   string strRequest = inattr("request_uri");
...
bool netnodeScriptImpl::locale_is_eu(const char * pszLocale)
{
   return System.url().locale_is_eu(pszLocale);
}

bool netnodeScriptImpl::locale_is_asia(const char * pszLocale)
{
   return System.url().locale_is_asia(pszLocale);
}

bool netnodeScriptImpl::locale_is_south_america(const char * pszLocale)
{
   return System.url().locale_is_south_america(pszLocale);
}

bool netnodeScriptImpl::locale_is_north_america(const char * pszLocale)
{
   return System.url().locale_is_north_america(pszLocale);
}

bool netnodeScriptImpl::locale_is_central_america(const char * pszLocale)
{
   return System.url().locale_is_central_america(pszLocale);
}

bool netnodeScriptImpl::locale_is_latin_america(const char * pszLocale)
{
   return System.url().locale_is_latin_america(pszLocale);
}

bool netnodeScriptImpl::locale_is_de(const char * pszLocale)
{
   string strLocale(pszLocale);
   if(strLocale == "be"
   || strLocale == "ch"
...
bool netnodeScriptImpl::locale_is_es(const char * pszLocale)
{
   string strLocale(pszLocale);
   if(strLocale == "es"
   || strLocale == "mx"
...
var netnodeScriptImpl::explode(const char * pszDelimiter, const char * psz)
{
   var var;
...
string netnodeScriptImpl::implode(const char * pszGlue, const var & var)
{
   return var.implode(pszGlue);
}

bool netnodeScriptImpl::in_array(const var & varElement, const var & varArray)
{
   return varArray.array_contains(varElement.get_string());
}

void netnodeScriptImpl::array_unshift(var & varArray, const var varElement)
{
   varArray.vara().insert_at(0, varElement);
}

var netnodeScriptImpl::array(
   var var01,
   var var02,
   var var03,
   var var04,
...
bool netnodeScriptImpl::is_array(var var)
{
   return var.is_array();
}

var netnodeScriptImpl::array_merge(
   var var01,
   var var02,
   var var03,
   var var04,
...
var netnodeScriptImpl::http_accept_language(const char * psz)
{
   string str(psz);
   if(str.is_empty())
   {
...
string netnodeScriptImpl::normalize_locale(const char * psz)
{
   string str(psz);
...
bool netnodeScriptImpl::defer_add_locale(const char * pszLocale, stringa & straLocale, stringa & straStyle)
{
   if(!straLocale.contains(pszLocale) && straLocale.get_size() < 26)
   {
      straLocale.add(pszLocale);
      if(straStyle.get_count() == 0)
      {
         if(gprop("param_style").get_string().is_empty()
         || gprop("param_style").get_string().CompareNoCase("_std") == 0)
         {
            straStyle.add("_std");
         }
         else
         {
            straStyle.add(gprop("param_style"));
         }
      }
      else
      {
...
void netnodeScriptImpl::add_locale_variant(stringa & straLocale, stringa & straStyle)
{
   string strLocale = straLocale[straLocale.get_upper_bound()];
   string strStyle = straStyle[straStyle.get_upper_bound()];
   stringa stra;
   ...
   if(stra.get_count() > 1)
   {
      ...
      {
         if(defer_add_locale(stra[i], straLocale, straStyle))
         {
            add_locale_variant(straLocale, straStyle);
         }
      }
   }
   else if(strLocale == "at")
   {   
      defer_add_locale("de-at", straLocale, straStyle);
      defer_add_locale("de", straLocale, straStyle);
      defer_add_locale("de-de", straLocale, straStyle);
   }
   else if(strLocale == "br")
   {   
      defer_add_locale("pt-br", straLocale, straStyle);
      defer_add_locale("pt", straLocale, straStyle);
      defer_add_locale("es", straLocale, straStyle);
   }
   else if(strLocale == "cn")
   {   
      defer_add_locale("zh-cn", straLocale, straStyle);
      defer_add_locale("zh-tw", straLocale, straStyle);
   }
   else if(strLocale == "hk")
   {   
      defer_add_locale("zh-cn", straLocale, straStyle);
      defer_add_locale("zh-tw", straLocale, straStyle);
      defer_add_locale("cn", straLocale, straStyle);
   }
...
void netnodeScriptImpl::process_final_locale_style(stringa & straLocale, stringa & straStyle)
{
restart:
   for(index i = 0; i < straLocale.get_count(); i++)
   {
      string strLocale = straLocale[i];
      string strStyle = straStyle[i];
      stringa stra;
      stra.explode("-", strLocale);
      if(stra.get_count() == 2)
      {
         if(straLocale.find_first(stra[0]) < 0)
         {
            straLocale.insert_at(i + 1, stra[0]);
            straStyle.insert_at(i + 1, strStyle);
            goto restart;
         }
         if(straLocale.find_first(stra[1]) < 0)
         {
            straLocale.insert_at(i + 1, stra[1]);
            straStyle.insert_at(i + 1, strStyle);
            goto restart;
         }
      }
   ...
   stringa straLocaleAdd1;
   stringa straStyleAdd1;
   if(gprop("g_script_left_to_right").is_set() && !(bool)gprop("g_script_left_to_right"))
   {
      for(index i = 0; i < straLocale.get_count(); i++)
      {
         ...
         {
            straLocaleAdd1.add(strLocale);
            straStyleAdd1.add(strStyle + "_rl");
         }
      }
   }
   if(gprop("g_script_left_to_right").is_set() && !(bool)gprop("g_script_left_to_right"))
   ...
   }
   {
      for(index i = 0; i < straLocale.get_count(); i++)
      {
         ...
         {
            straLocaleAdd1.add(strLocale);
            straStyleAdd1.add("_std");
         }
      }
   }
   straLocale.add(straLocaleAdd1);
   straStyle.add(straStyleAdd1);
}

int netnodeScriptImpl::count(var var)
{
   if(var.is_array())
...
var netnodeScriptImpl::emphasize_text(var textArg, var emphasizeArg)
{
   string text = textArg;
...
string netnodeScriptImpl::spa_calc_best_host()
{
...
GeoIPRecord * netnodeScriptImpl::get_client_geopip_record()
{
...
void netnodeScriptImpl::echo_simage(var param_view)
{
   echo(simage(param_view));
}
   
string netnodeScriptImpl::simage(var param_view)
{
   string base_url = "http://veriterse.net/";
   return base_url + simage_relative(param_view);
}
   
void netnodeScriptImpl::simage_add_accepta(netnode::simage_accepta * paccepta)
{
   get_manager()->m_simageptra.add(paccepta);
}   
   
string netnodeScriptImpl::simage_relative(var param_view)
{
   netnode::simage_accepta_ptr_array simageptra = get_manager()->m_simageptra;
   string candidate;
   string path;
   for(int i = 0; i < simageptra.get_count(); i++)
   {
      netnode::simage_accepta * paccepta = simageptra[i];
...
bool netnodeScriptImpl::net_hostname_is_spider(const char * hostname, const char * useragent)
{
   CSingleLock sl(&get_manager()->m_mutexSpider, TRUE);
   match_host_ptr_array & ptra = get_manager()->m_matchhostaSpider;
   for(int i = 0; i < ptra.get_count(); i++)
   {
      if(ptra[i]->matches(hostname, useragent))
         return true;
   }
   return false;
}
...
void netnodeScriptImpl::add_spider_match_host(const char * hostname, const char * useragent)
{
   CSingleLock sl(&get_manager()->m_mutexSpider, TRUE);
   match_host_ptr_array & ptra = get_manager()->m_matchhostaSpider;
   ptra.add(new match_host(hostname, useragent));

}

string netnodeScriptImpl::ca2_html_fluidcode(const char * pszFreeText)
{
   stringa stra;
...
string netnodeScriptImpl::query_ca2(const char * pszQuery)
{
   stringa straResp;
   string strResp;
   {
   ...
   try
   {
      ...
      if(PcreUtil::match(str_context(), stra, strQuery, "/(.+)\\s*%1/", "calendar:days"))
      {
         ...
         class time time = System.datetime().strtotime(
            str_context(),
            strSignal + " " + gen::str::itoa(pelement->get_value().mod()) + " days",
            0, iPathCount);
         string strValue = System.datetime().international().get_gmt_date_time(time);
         strResp = "<a href=\"calendar?query=" + urlencode(strSignal + " " + gen::str::itoa(pelement->get_value().mod())) + "+days"+"\">";
   ...
   try
   {
      ...
      do
      {
         ...
         datetime::parser parser(get_app());
...
void netnodeScriptImpl::ca2_query_date(stringa & straResp, class time & time, const char * pszDateTime, const char * pszQuery)
{
   ...
   if(gen::str::find_ci(unitext("aniversario"), pszQuery) |
      gen::str::find_ci(unitext("niver"), pszQuery) |
      gen::str::find_ci(unitext("níver"), pszQuery) |
      gen::str::find_ci(unitext("aniversário"), pszQuery) |
      gen::str::find_ci(unitext("aniversÁrio"), pszQuery) |
      gen::str::find_ci(unitext("nÍver"), pszQuery) |
      gen::str::find_ci(unitext("Geburtstag"), pszQuery))
   {
      strTopic = pszQuery;
      strTopic = gen::str::replace_ci(unitext("aniversario"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("niver"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("níver"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("aniversário"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("aniversÁrio"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("nÍver"), "", strTopic);
      strTopic = gen::str::replace_ci(unitext("Geburtstag"), "", strTopic);
      ...
      strResp += unitext("Aniversário: ") + gen::str::itoa(iDay) + ", " + pstr("datetimestr_month["+gen::str::itoa(iMonth - 1)+"]");
      strResp += ", ";
...
netnodeScriptInterface & netnodeScriptImpl::main()
{
   return *m_pinstanceMain;
}


::user::str_context * netnodeScriptImpl::str_context()
{
   if(this == m_pinstanceMain)
...
void netnodeScriptImpl::print_email_body(::mail::pop3 * ppop3)
{
...
void netnodeScriptImpl::print_plain_text_email_body(::mail::pop3 * ppop3)
{
...
void netnodeScriptImpl::ob_start()
{
   m_pinstanceMain->m_memoryfileaPrint.add_new();
}

primitive::memory netnodeScriptImpl::ob_get_clean()
{
...
::http::file & netnodeScriptImpl::output_file()
{
...
void netnodeScriptImpl::filter_user_presence_status(const char * pszUserId)
{
...
bool netnodeScriptImpl::is_licensed(const char * pszLicense, const char * pszUserId)
{
...
string netnodeScriptImpl::web_map(const char * psz)
{
...
bool netnodeScriptImpl::is_site_user(var varSite, var varUser)
{
   return musicdb().query_item("SELECT COUNT(*) FROM hi5.site_user WHERE site = " + varSite + " AND user = " + varUser) == 1;
}

bool netnodeScriptImpl::is_site_user(var varSite)
{
   return is_site_user(varSite, gprop("secureuserid"));
}


bool netnodeScriptImpl::low_fs_read(const char * user, __int64 iFolder, const char * pszName)
{
   string strSql;
...
string netnodeScriptImpl::low_fs_file_path(const char * hash, __int64 key, __int64 size, const char * mimetype, const char * extension)
{
   string strSql;
...
bool netnodeScriptImpl::low_fs_read(const char * hash, __int64 key, __int64 size, const char * mimetype, const char * extension)
{
   return read_file(low_fs_file_path(hash, key, size, mimetype, extension));

}

__int64 netnodeScriptImpl::low_fs_map(const char * pszDir)
{
   stringa stra;
...
bool netnodeScriptImpl::low_fs_map_file(const char * pszFile)
{
   string strSql;
...
bool netnodeScriptImpl::low_fs_license(const char * user, const char * hash, __int64 key, __int64 size, const char * mimetype, const char * extension)
{
   string strName;
...
bool netnodeScriptImpl::low_fs_is_licensed(const char * user, const char * hash, __int64 key, __int64 size, const char * mimetype, const char * extension)
{
   string strSql;
...
bool netnodeScriptImpl::low_fs_add_user_file(const char * user, __int64 iFolder, const char * hash, __int64 key, __int64 size, const char * mimetype, const char * extension, const char * pszName, bool bOverwrite)
{
   string strSql;
   string strSqlPrefix;
   ...
   strSql.Format(strSqlPrefix + " `fs`.`user_folder_item`(`user`,  `folder`, `hash`, `key`, `size`, `mimetype`, `extension`, `name`) VALUES ('%s', '%I64d', '%s', '%I64d', '%I64d', '%s', '%s', '%s')",
      user, iFolder, hash, key, size, mimetype, extension, pszName);
...
__int64 netnodeScriptImpl::low_fs_add_user_dir(const char * user, __int64 iFolder, const char * pszName)
{
   string strSql;
...
void netnodeScriptImpl::low_fs_ls_dir(const char * user, __int64 iFolder, int64_array & ia, stringa & stra)
{
   string strSql;
...
void netnodeScriptImpl::low_fs_ls_file(const char * user,  __int64 iFolder, stringa & straHash, int64_array & iaKey, int64_array & iaSize, stringa & straMime, stringa & straExt, stringa & straName)
{
   string strSql;
...
bool netnodeScriptImpl::low_folder(string & strUser, __int64 & iFolder, const char * pszFolder)
{
...
bool      netnodeScriptImpl::fs_dir_mk               (const char * pszFolder)
   {
string strPath(pszFolder);
string strUser;
__int64 iFolder;
...
int netnodeScriptImpl::datetime_get_month_day_count(int year, int month)
{
	switch(month)
	{
	case 1:
...
// 0 is Sunday
string netnodeScriptImpl::datetime_get_weekday(int year, int month, int day)
{
	return strftime("%w", datetime_get_timestamp(year, month, day).get_time());
}

class time netnodeScriptImpl::datetime_get_timestamp(int year, int month, int day)
{
...
string netnodeScriptImpl::datetime_formatDateTime(class time t)
{
	return t.FormatGmt("Y-m-d H:i:s");
}

class time netnodeScriptImpl::datetime_getTimestamp()
{
   return time::get_current_time();
}

__int64 netnodeScriptImpl::datetime_InternationalToUnixMillis(const char * pszDatetime)
{
...
class time netnodeScriptImpl::datetime_strtotime(const char * psz)
{
   return datetime_strtotime(psz, time::get_current_time());
}

class time netnodeScriptImpl::datetime_strtotime(const char * psz, class time t)
{
   class time datetime(strtotime(psz, t.get_time()));
   return class time(datetime.GetGmtYear(), datetime.GetGmtMonth(), datetime.GetGmtDay());
}
