...
public:
	...
	virtual WINBOOL isValid() const = 0;
	//@}
};
...
public:
	...
	*/
	...
	@see FreeImage_AllocateT
	*/
	WINBOOL setSize(FREE_IMAGE_TYPE image_type, unsigned width, unsigned height, unsigned bpp, unsigned red_mask = 0, unsigned green_mask = 0, unsigned blue_mask = 0);
	/// Destroy image data
	virtual void clear();
	//@}
	...
	*/
	...
	@see FreeImage_Copy
	*/
	WINBOOL copySubImage(fipImage& dst, int left, int top, int right, int bottom) const;
	...
	@see FreeImage_Paste
	*/
	WINBOOL pasteSubImage(fipImage& src, int left, int top, int alpha = 256);
	...
	@return Returns TRUE if successful, FALSE otherwise.
	*/
	WINBOOL crop(int left, int top, int right, int bottom);
	...
	*/
	WINBOOL load(const char* lpszPathName, int flag = 0);

	/**
	UNICODE version of load (this function only works under WIN32 and does nothing on other OS)
	@see load
	*/
	WINBOOL loadU(const wchar_t* lpszPathName, int flag = 0);
	...
	*/
	WINBOOL loadFromHandle(FreeImageIO *io, fi_handle handle, int flag = 0);
	...
	*/
	WINBOOL loadFromMemory(fipMemoryIO& memIO, int flag = 0);
	...
	*/
	WINBOOL save(const char* lpszPathName, int flag = 0) const;

	/**
	UNICODE version of save (this function only works under WIN32 and does nothing on other OS)
	@see save
	*/
	WINBOOL saveU(const wchar_t* lpszPathName, int flag = 0) const;
	...
	*/
	WINBOOL saveToHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flag = 0) const;
	...
	*/
	WINBOOL saveToMemory(FREE_IMAGE_FORMAT fif, fipMemoryIO& memIO, int flag = 0) const;
	...
	*/
	WINBOOL getThumbnail(fipImage& image) const;
	...
	*/
	WINBOOL setThumbnail(const fipImage& image);
	...
	*/
	WINBOOL getPixelIndex(unsigned x, unsigned y, BYTE *value) const;
	...
	*/
	WINBOOL getPixelColor(unsigned x, unsigned y, RGBQUAD *value) const;
	...
	*/
	WINBOOL setPixelIndex(unsigned x, unsigned y, BYTE *value);
	...
	*/
	WINBOOL setPixelColor(unsigned x, unsigned y, RGBQUAD *value);
	...
	*/
	WINBOOL convertToType(FREE_IMAGE_TYPE image_type, WINBOOL scale_linear = TRUE);
	...
	*/
	WINBOOL dither(FREE_IMAGE_DITHER algorithm);
	...
	*/
	WINBOOL toneMapping(FREE_IMAGE_TMO tmo, double first_param = 0, double second_param = 0, double third_param = 1, double fourth_param = 0);
	...
	*/
	WINBOOL getFileBkColor(RGBQUAD *bkcolor) const;
	...
	*/
	WINBOOL setFileBkColor(RGBQUAD *bkcolor);
	//@}
	...
	*/
	WINBOOL getChannel(fipImage& image, FREE_IMAGE_COLOR_CHANNEL channel) const;
	...
	*/
	WINBOOL setChannel(fipImage& image, FREE_IMAGE_COLOR_CHANNEL channel);
	...
	*/
	WINBOOL splitChannels(fipImage& RedChannel, fipImage& GreenChannel, fipImage& BlueChannel);
	...
	*/
	WINBOOL combineChannels(fipImage& red, fipImage& green, fipImage& blue);
	//@}
	...
	*/
	WINBOOL rotateEx(double angle, double x_shift, double y_shift, double x_origin, double y_origin, WINBOOL use_mask);
	...
	*/
	WINBOOL rotate(double angle, const void *bkcolor = NULL);

	/**
	Flip the image horizontally along the vertical axis
	@see FreeImage_FlipHorizontal
	*/
	WINBOOL flipHorizontal();

	/**
	Flip the image vertically along the horizontal axis
	@see FreeImage_FlipVertical
	*/
	WINBOOL flipVertical();
	//@}
	...
	*/
	WINBOOL adjustCurve(BYTE *LUT, FREE_IMAGE_COLOR_CHANNEL channel);
	...
	*/
	WINBOOL adjustGamma(double gamma);
	...
	*/
	WINBOOL adjustBrightness(double percentage);
	...
	*/
	WINBOOL adjustContrast(double percentage);
	...
	*/
	WINBOOL adjustBrightnessContrastGamma(double brightness, double contrast, double gamma);
	...
	*/
	WINBOOL getHistogram(DWORD *histo, FREE_IMAGE_COLOR_CHANNEL channel = FICC_BLACK) const;
	//@}
	...
	*/
	WINBOOL rescale(unsigned new_width, unsigned new_height, FREE_IMAGE_FILTER filter);
	...
	*/
	WINBOOL makeThumbnail(unsigned max_size, WINBOOL convert = TRUE);
	//@}
	...
	@see isModified
	*/
	void setModified(WINBOOL bStatus = TRUE) {
	...
	*/
	WINBOOL getMetadata(FREE_IMAGE_MDMODEL model, const char *key, fipTag& tag) const;
	/**
	...
	*/
	WINBOOL setMetadata(FREE_IMAGE_MDMODEL model, const char *key, fipTag& tag);
	//@}
  ...
  protected:
	/**@name Internal use */
	//@{
	  WINBOOL replace(FIBITMAP *new_dib);
	//@}
...
public:
	...
	*/
	...
	@return Returns TRUE if successful, returns FALSE otherwise
	*/
	WINBOOL copyFromHandle(HANDLE hMem);

	/** Copy constructor.<br>
	Converts a HBITMAP object to a FIBITMAP.
	@return Returns TRUE if successful, returns FALSE otherwise
	*/
	WINBOOL copyFromBitmap(HBITMAP hbmp);
	//@}
	...
	@return Returns TRUE if successful, returns FALSE otherwise
	*/
	WINBOOL copyToClipboard(HWND hWndNewOwner) const;

	/**
	Retrieves data from the clipboard. The clipboard format must be CF_DIB.
	@return Returns TRUE if successful, returns FALSE otherwise
	*/
	WINBOOL pasteFromClipboard();
	//@}
	...
	@return Returns TRUE if successful, returns FALSE otherwise
	*/
	WINBOOL captureWindow(HWND hWndApplicationWindow, HWND hWndSelectedWindow);
	//@}
	...
	*/
	void drawEx(HDC hDC, RECT& rcDest, WINBOOL useFileBkg = FALSE, RGBQUAD *appBkColor = NULL, FIBITMAP *bg = NULL) const;
...
public :
	...
	*/
	WINBOOL save(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, int flags = 0);
	/**
	...
	*/
	WINBOOL saveMultiPage(FREE_IMAGE_FORMAT fif, FIMULTIBITMAP *bitmap, int flags = 0);
	/**
	...
	*/
	WINBOOL seek(long offset, int origin);
	/**
	...
	*/
	WINBOOL acquire(BYTE **data, DWORD *size_in_bytes);
	//@}
...
public:
	...
	*/
	fipMultiPage(WINBOOL keep_cache_in_memory = FALSE);
	...
	*/
	WINBOOL open(const char* lpszPathName, WINBOOL create_new, WINBOOL read_only, int flags = 0);
	...
	@see FreeImage_LoadMultiBitmapFromMemory
	*/
	WINBOOL open(fipMemoryIO& memIO, int flags = 0);
	...
	*/
	WINBOOL open(FreeImageIO *io, fi_handle handle, int flags = 0);
	...
	*/
	WINBOOL saveToHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flags = 0) const;
	...
	*/
	WINBOOL saveToMemory(FREE_IMAGE_FORMAT fif, fipMemoryIO& memIO, int flags = 0) const;
	...
	*/
	WINBOOL movePage(int target, int source);
	...
	*/
	void unlockPage(fipImage& image, WINBOOL changed);
	...
	*/
	WINBOOL getLockedPageNumbers(int *pages, int *count) const;
};
...
public:
	...
	*/
	...
	@see FreeImage_CreateTag
	*/
	WINBOOL setKeyValue(const char *key, const char *value);
...
public:
	...
	*/
	WINBOOL findFirstMetadata(FREE_IMAGE_MDMODEL model, fipImage& image, fipTag& tag);
	/**
	...
	*/
	WINBOOL findNextMetadata(fipTag& tag);
