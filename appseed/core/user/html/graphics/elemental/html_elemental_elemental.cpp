...
namespace html
{
   ...
   void elemental::initialize()
   {
      ...
      m_etag = m_papp->m_psystem->m_phtml->tag_name_to_id(m_idTagName);
...
//      e_tag etype = m_etag;
   ...
   void elemental::load(data * pdata, base * pbase)
   {
      ...
      if (m_pbase->get_type() == base::type_tag)
      {
         ...
         if (m_idTagName == __id(html_link) && get_tag()->get_attr_value("rel").compare_ci("stylesheet") == 0)
         {
            ...
            sp(style_sheet) pstylesheet(canew(style_sheet(m_papp)));
   ...
   bool elemental::parse(data * pdata, const char * & pszParam)
   {
      ...
      if (pszCloseTag[0] == '/' && strnicmp_dup(m_idTagName.m_psz, pszCloseTag + 1, psz - pszCloseTag - 1) == 0)
      {
         psz++;
         pszParam = psz;
         return true;
   ...
   void elemental::get_html(data * pdata, string & str)
   {
      ...
      if (m_pbase->get_type() == base::type_value)
      {
         ...
         str += Sys(pdata->m_papp).m_phtml->entities(m_strBody);
