...
namespace html
{
   ...
   elemental::elemental(data * pdata, elemental * pparent) :
      m_style(pdata->m_pauraapp)
   {

      m_pauraapp = pdata->m_pauraapp;
      m_pparent = pparent;
      m_pimpl = NULL;
      m_pbase = NULL;
      m_pdata = pdata;
   ...
   void elemental::initialize()
   {
      ...
      m_etag = m_pauraapp->m_pcoresystem->m_phtml->tag_name_to_id(m_idTagName);
