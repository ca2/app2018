...
namespace rar
{
   ...
   {
      throw new input_file_exception(get_app(), cause);
   }
   ...
   HRESULT input_file::ReadBytesSpec(void * data, memory_size_t * resSize)
   {
      if (m_CryptoMode)
      {
         memory_size_t size = *resSize;
         *resSize = 0;
         const byte *bufData = m_DecryptedDataAligned;
         memory_size_t bufSize = m_DecryptedDataSize;
         memory_size_t i;
   ...
   bool input_file::ReadBytesAndTestSize(void *data, memory_size_t size)
   {
   ...
   HRESULT input_file::Open2(::file::input_stream *stream, const file_position_t *searchHeaderSizeLimit)
   {
      ...
      memory_size_t commentSize = blockSize - headerSize;
   ...
   byte input_file::ReadByte()
   {
      if (m_CurPos >= m_PosLimit)
         throw new input_file_exception(get_app(), input_file_exception::kIncorrectArchive);
   ...
   HRESULT input_file::GetNextItem(CItemEx &item, ::crypto::get_text_password_interface *getTextPassword, bool &decryptionError, string &errorMessage)
   {
      ...
      for (;;)
      {
            ...
            header::archive::kBlockHeadersAreEncrypted) != 0)
         {
            ...
            RINOK(m_RarAES->Init());
            memory_size_t decryptedDataSizeT = kDecryptedBufferSize;
