...
void biteditor_view::install_message_handling(::user::win::message::dispatch * pinterface)
{
...
//   biteditor_document * pdoc = get_document();
   ...
   size size1 = pdc->GetTextExtent("gqYALÍ");
   
   pdc->SelectObject(m_pasciibox->m_spfont);
   size size2 = pdc->GetTextExtent("gqYALÍ");
   ...
   int iLineCount = min(m_iViewSize, get_document()->m_peditfile->get_length()) / m_iLineSize;

   iLineCount += 4;

   int iHeight = m_iLineHeight * iLineCount;

   class size sizeHex;
   class size sizeAsc;

   sizeHex.cx = m_iLineSize * 3 * size1.cx / 6 + 20;
   sizeAsc.cx = m_iLineSize * size2.cx / 6 + 20;
...
void biteditor_view::_001OnCreate(gen::signal_object * pobj) 
{
   ...
   m_pasciibox->create(this, "ascii_box");
   m_phexbox->create(this, "hex_box");
...
int64_t biteditor_view::CalcLineCount()
{
...
void biteditor_view::get_view_lines(stringa & straLines)
{
   file_offset iOffset = (m_iViewOffset / m_iLineSize * m_iLineSize);

   get_document()->m_memfile.seek(iOffset, ex1::seek_begin);

   int iLineCount = m_iViewSize / m_iLineSize;
...
void biteditor_view::_001GetSelText(string & str) const
{
   file_position iEnd;
   ...
   get_document()->m_peditfile->seek((file_offset) iStart, ::ex1::seek_begin);
