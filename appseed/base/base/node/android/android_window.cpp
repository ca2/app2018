...
//               pui->send_message(WM_MOUSELEAVE);
   ...
   sp(::ca2::window) pWnd = ::android::window::FromHandlePermanent(wndTemp.get_handle());
   if (pWnd == NULL)
   {
   pWnd = &wndTemp;
   }
...
*/
   ...
   void PASCAL window::SendMessageToDescendants(void * hWnd, UINT message, WPARAM wparam, lparam lparam, bool bDeep, bool bOnlyPerm)
   {
         ...
         hWndChild = ::GetNextWindow(hWndChild, GW_HWNDNEXT))*/
      {
         ...
         /*if (bOnlyPerm)
         {
            sp(::ca2::window) pWnd = ::android::window::FromHandlePermanent(hWndChild);
            if (pWnd != NULL)
            {
...
//      return bSuccess;
   ...
   id window::RunModalLoop(DWORD dwFlags, ::ca2::live_object * pliveobject)
   {
      ...
      sp(::base::application) pappThis1 =  (m_pthread->m_pthread->m_p);
      sp(::base::application) pappThis2 =  (m_pthread->m_pthread);
...
//   HINSTANCE hInst = Sys(::android::get_thread()->m_papp).m_hInstance;
//
...
//   wndcls.lpfnWndProc = DefWindowProc;
//   wndcls.hInstance = Sys(::android::get_thread()->m_papp).m_hInstance;
...
//            DWORD dwStyle;
//            rect rectOld;
//            sp(::ca2::window) pWnd = ::android::window::from_handle(hWnd);
