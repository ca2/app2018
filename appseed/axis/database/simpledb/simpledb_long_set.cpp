...
//bool db_long_set::load(const ::database::key & key, int64_t * plValue)
//{
//
...
//
//      if (pcore->m_map.Lookup(key.m_str, longitem) && longitem.m_dwTimeout > get_tick_count())
//      {
//
...
//      strUrl += System.url().url_encode(key.m_str);
//
...
//
//      pcore->m_map.set_at(key.m_str, longitem);
//      return true;
//
//   }
//#ifndef METROWIN
...
//      {
//
//         *plValue = pcore->m_psimpledbUser->query_item("SELECT `value` FROM fun_user_str_set WHERE user = '" + pcore->m_strUser + "' AND `key` = '" + pcore->m_psimpledbUser->real_escape_string(key.m_str) + "'").i32();
//
//         return true;
//
//      }
...
//}
//
//bool db_long_set::save(const ::database::key & key, int64_t lValue)
//{
//
//   db_long_set_core * pcore = (db_long_set_core *)m_pcore->m_ptopthis;
//
...
//      }
//
//      pcore->m_pqueue->queue(key.m_str, lValue);
//
//      db_long_set_item longitem;
//
//      longitem.m_dwTimeout = get_tick_count() + 23 * (5000);
...
//   {
//
//      string strSql = "REPLACE INTO fun_user_long_set VALUE('" + pcore->m_strUser + "', '" + pcore->m_psimpledbUser->real_escape_string(key.m_str) + "', " + ::str::from(lValue) + ")";
//
//      TRACE(strSql);
//
...
//}
//
//bool db_long_set::find(const ::database::key & key)
//{
...
//
//
//bool db_long_set::load(const ::database::key & key, LPPOINT lpPoint)
//{
//
//   point point;
//
//   string strKey = key.m_str;
//
