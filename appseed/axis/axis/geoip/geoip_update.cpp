...
int32_t GeoIP_fprintf(int32_t (*f)(FILE *, char *),FILE *fp, const char *str, ...) {
  va_list ap;
  int32_t rc;
  char * f_str;
  int32_t silence;
...
//   for (;;) {
//      int32_t amt;
//      amt = (int32_t) recv(sock, &buf[offset], block_size,0);
//      if (amt == 0) {
//         break;
//      } else if (amt == -1) {
//         free(buf);
...
//      int32_t netspeed = GeoIP_id_by_name(gi,"24.24.24.24");
//      lookupresult = 0;
