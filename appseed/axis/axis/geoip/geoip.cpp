...
static const uint32_t num_GeoIP_countries = (uint32_t)(sizeof(GeoIP_country_code)/sizeof(GeoIP_country_code[0]));
...
int32_t __GEOIP_V6_IS_NULL(geoipv6_t v6) {
   int32_t i;
   for (i=0;i<16;i++) {
      if (v6.s6_addr[i])
         return 0;
      ...
      if (stat(gi->file_path, &buf) != -1) {
         if (buf.st_mtime != gi->mtime) {
            ...
            if ( gi->flags & GEOIP_MMAP_CACHE) {
               ...
               return -1;
#else
               gi->cache = (uchar *) mmap(NULL, buf.st_size, PROT_READ, MAP_PRIVATE, _fileno(gi->GeoIPDataaxis), 0);
...
uint32_t _GeoIP_seek_record_v6 (GeoIP *gi, geoipv6_t ipnum) {
   ...
   const uchar *buf = (gi->cache == NULL) ? stack_buffer : NULL;
   uint32_t offset = 0;
...
uint32_t _GeoIP_seek_record (GeoIP *gi, uint32_t ipnum) {
   int32_t depth;
   uint32_t x;
   uchar stack_buffer[2 * MAX_RECORD_LENGTH];
   const uchar *buf = (gi->cache == NULL) ? stack_buffer : NULL;
...
GeoIP* GeoIP_open_type (int32_t type, int32_t flags) {
   GeoIP * gi;
   const char * filePath;
   if (type < 0 || type >= NUM_DB_TYPES) {
...
GeoIP* GeoIP_open (const char * filename, int32_t flags) {
   struct stat buf;
   GeoIP * gi;
   size_t len;
...
int32_t GeoIP_id_by_name (GeoIP* gi, const char *name) {
   uint32_t ipnum;
   int32_t ret;
   if (name == NULL) {
      return 0;
   ...
   if (gi->databaseType != (char) GEOIP_COUNTRY_EDITION && gi->databaseType != (char) GEOIP_PROXY_EDITION && gi->databaseType != (char) GEOIP_NETSPEED_EDITION) {
      debug_print("Invalid database type %s, expected %s\n", GeoIPDBDescription[(int32_t)gi->databaseType], GeoIPDBDescription[GEOIP_COUNTRY_EDITION]);
      return 0;
   }
...
int32_t GeoIP_id_by_name_v6 (GeoIP* gi, const char *name) {
   geoipv6_t ipnum;
   int32_t ret;
   if (name == NULL) {
      return 0;
   ...
   if (gi->databaseType != (char) GEOIP_COUNTRY_EDITION_V6 && gi->databaseType != (char) GEOIP_PROXY_EDITION && gi->databaseType != (char) GEOIP_NETSPEED_EDITION) {
      debug_print("Invalid database type %s, expected %s\n", GeoIPDBDescription[(int32_t)gi->databaseType], GeoIPDBDescription[GEOIP_COUNTRY_EDITION_V6]);
      return 0;
   }
...
const char *GeoIP_country_name_by_ipnum (GeoIP* gi, uint32_t ipnum) {
   int32_t country_id;
...
const char *GeoIP_country_code_by_ipnum (GeoIP* gi, uint32_t ipnum) {
   int32_t country_id;
   country_id = GeoIP_id_by_ipnum(gi, ipnum);
   return (country_id > 0) ? GeoIP_country_code[country_id] : NULL;
}
...
const char *GeoIP_country_code3_by_ipnum (GeoIP* gi, uint32_t ipnum) {
   int32_t country_id;
   country_id = GeoIP_id_by_ipnum(gi, ipnum);
   return (country_id > 0) ? GeoIP_country_code3[country_id] : NULL;
}
...
int32_t GeoIP_country_id_by_addr (GeoIP* gi, const char *addr) {
   return GeoIP_id_by_addr(gi, addr);
}

int32_t GeoIP_country_id_by_name (GeoIP* gi, const char *host) {
   return GeoIP_id_by_name(gi, host);
}

int32_t GeoIP_id_by_addr_v6 (GeoIP* gi, const char *addr) {
   geoipv6_t ipnum;
   int32_t ret;
   if (addr == NULL) {
      return 0;
   ...
   if (gi->databaseType != (char) GEOIP_COUNTRY_EDITION_V6 &&
      ...
      gi->databaseType != (char) GEOIP_NETSPEED_EDITION) {
         debug_print("Invalid database type %s, expected %s\n",
            GeoIPDBDescription[(int32_t)gi->databaseType],
...
int32_t GeoIP_id_by_addr (GeoIP* gi, const char *addr) {
   uint32_t ipnum;
   int32_t ret;
   if (addr == NULL) {
      return 0;
...
int32_t GeoIP_id_by_ipnum_v6 (GeoIP* gi, geoipv6_t ipnum) {
   int32_t ret;
   /*     if (ipnum == 0) {
   return 0;
   }
...
int32_t GeoIP_id_by_ipnum (GeoIP* gi, uint32_t ipnum) {
   int32_t ret;
   if (ipnum == 0) {
      return 0;
   }
...
void GeoIP_assign_region_by_inetaddr(GeoIP* gi, uint32_t inetaddr, GeoIPRegion *region) {
...
GeoIPRegion * GeoIP_region_by_addr (GeoIP* gi, const char *addr) {
   ...
   if (gi->databaseType != (char) GEOIP_REGION_EDITION_REV0 &&
      gi->databaseType != (char) GEOIP_REGION_EDITION_REV1) {
         debug_print("Invalid database type %s, expected %s\n", GeoIPDBDescription[(int32_t)gi->databaseType], GeoIPDBDescription[GEOIP_REGION_EDITION_REV1]);
         return 0;
   }
   ipnum = _GeoIP_addr_to_num(addr);
...
GeoIPRegion * GeoIP_region_by_ipnum (GeoIP* gi, uint32_t ipnum) {
   if (gi->databaseType != (char) GEOIP_REGION_EDITION_REV0 &&
      gi->databaseType != (char) GEOIP_REGION_EDITION_REV1) {
         debug_print("Invalid database type %s, expected %s\n", GeoIPDBDescription[(int32_t)gi->databaseType], GeoIPDBDescription[GEOIP_REGION_EDITION_REV1]);
         return 0;
...
char *_get_name (GeoIP* gi, uint32_t ipnum) {
   ...
   if (gi->databaseType != (char) GEOIP_ORG_EDITION &&
      gi->databaseType != (char) GEOIP_ISP_EDITION &&
      gi->databaseType != (char) GEOIP_ASNUM_EDITION) {
         debug_print("Invalid database type %s, expected %s\n", GeoIPDBDescription[(int32_t)gi->databaseType], GeoIPDBDescription[GEOIP_ORG_EDITION]);
         return NULL;
   }
...
char *GeoIP_name_by_ipnum (GeoIP* gi, uint32_t ipnum) {
...
char *GeoIP_org_by_ipnum (GeoIP* gi, uint32_t ipnum) {
...
int32_t GeoIP_charset( GeoIP* gi){
   return gi->charset;
}

int32_t GeoIP_set_charset(  GeoIP* gi, int32_t charset ){
   int32_t old_charset = gi->charset;
   gi->charset = charset;
   return old_charset;
}

int32_t GeoIP_last_netmask (GeoIP* gi) {
   return gi->netmask;
}


/** return two letter country code */
const char* GeoIP_code_by_id(int32_t id)
{
   if (id < 0 || id >= (int32_t) num_GeoIP_countries)
      return NULL;
...
const char* GeoIP_code3_by_id(int32_t id)
{
   if (id < 0 || id >= (int32_t) num_GeoIP_countries)
      return NULL;
...
const char* GeoIP_name_by_id(int32_t id)
{
   if (id < 0 || id >= (int32_t) num_GeoIP_countries)
      return NULL;
...
const char* GeoIP_continent_by_id(int32_t id)
{
   if (id < 0 || id >= (int32_t) num_GeoIP_countries)
      return NULL;
...
int32_t GeoIP_id_by_code(const char *country)
{
   uint32_t i;
