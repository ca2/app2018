...
namespace sockets
{
...
#endif
            SetCloseAndDelete();
            _throw(simple_exception(get_app(), "bind() failed for udp_socket, port:range: " + ::str::from(ad.get_service_number()) + ":" + ::str::from(range)));
            return -1;
         }
         m_bind_ok = true;
   ...
   bool udp_socket::open(const ::net::address & ad)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         {
#ifdef DEBUG
            log("connect", Errno, bsd_socket_error(Errno), ::aura::log::level_fatal);
#endif
            SetCloseAndDelete();
            return false;
         }
...
#ifdef DEBUG
         log("write", 0, "not connected", ::aura::log::level_error);
#endif
         return;
...
#ifdef DEBUG
         log("write", Errno, bsd_socket_error(Errno), ::aura::log::level_error);
#endif
