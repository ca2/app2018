...
//		m_objecta.clear();
//		_throw(invalid_argument_exception("event_collection: too many wait objects"));
//	}
//}
...
void event_collection::remove(index index)
{
   ...
   if ( index > m_objecta.size() )
      _throw(range_error(get_app(), "event_collection::remove: index out of bounds"));
...
#else
               winResult = ::WaitForMultipleObjectsEx(static_cast<uint32_t>(m_objecta.size()), (sync_object **) &*m_objecta.begin(), waitForAll, start + timeout - ticks, true);
#endif
...
#else
         winResult = ::WaitForMultipleObjectsEx(static_cast<uint32_t>(m_objecta.size()),(sync_object **)&*m_objecta.begin(),waitForAll,0,FALSE);
#endif
...
wait_result event_collection::find_next( const wait_result& result ) const
{
   if ( !result.abandoned() && !result.signaled() )
      _throw(range_error(get_app(), "no element signaled"));
...
event_collection::event_collection( const event_collection& )
{
   ...
   _throw(void_implementation_exception(get_app()));
