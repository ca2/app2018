...
public:
   ...
   memory_size_t read_inline(void *lpBuf, memory_size_t nCount)
   {
      ...
      if (nCount > (memory_size_t) iDiff)
         nCount = iDiff;

      if (nCount == 1)
      {

         *((byte*)lpBuf) = m_spmemory.m_p->m_pbComputed[m_dwPosition];

      }
      else if (nCount == 2)
      {

         *((uint16_t *)lpBuf) = *((uint16_t *)&m_spmemory.m_p->m_pbComputed[m_dwPosition]);

      }
      else if (nCount == 4)
      {

         *((uint32_t*)lpBuf) = *((uint32_t*)&m_spmemory.m_p->m_pbComputed[m_dwPosition]);

      }
      else if (nCount == 8)
      {

         *((uint64_t *)lpBuf) = *((uint64_t *)&m_spmemory.m_p->m_pbComputed[m_dwPosition]);

      }
      else
      {

         memcpy(lpBuf, &m_spmemory.m_p->m_pbComputed[m_dwPosition], (size_t)nCount);
