...
namespace file
{
   ...
   file_position_t buffered_file::seek(file_offset_t lOff, ::file::e_seek nFrom)
   {
      ...
      else if(nFrom == ::file::seek_end)
      {
         ...
         //if (comparison::gt(uiNewPos, m_pfile->get_length()))
      ...
      else if(nFrom == ::file::seek_current)
      {
         ...
         if(iNewPos < 0)
            iNewPos = 0;
         else if(::comparison::gt(iNewPos, m_pfile->get_length()))
            iNewPos = m_pfile->get_length();
         uiNewPos = iNewPos;
      }
      else
      {
         throw new invalid_argument_exception(get_app(), "::file::buffered_file::seek invalid seek option");
      }
