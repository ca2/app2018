...
bstring::bstring(const bstring& str, strsize pos, strsize len)
{
   ...
   memcpy(m_puchData, &str.m_puchData[pos], get_allocation_size());
...
bstring & bstring::assign(const bstring & str, strsize pos, strsize len)
{
   ...
   memcpy(m_puchData, &str.m_puchData[pos], len);
...
bstring & bstring::append(strsize s, char c)
{
   ...
   if (s > 0)
   {
      ...
      memset(&m_puchData[sPrevious], c, s);
...
bstring & bstring::erase(strsize s, strsize len)
{
   ...
   else
   {
      ...
      memmove(&m_puchData[s], &m_puchData[s + len], length() - s - len);
...
void bstring::append(const void * p, strsize s)
{
   ...
   memcpy(&m_puchData[sPrevious - 1], p, s);
...
bstring bstring::operator + (const char * psz) const
{
   ...
   if (psz != NULL)
   {
      ...
      memcpy(&bstr[bstr.get_size() - 1 - strlen(psz)], psz, strlen(psz));
      bstr[bstr.get_size() - 1] = '\0';
...
bstring bstring::operator + (const bstring & bstr2) const
{
   ...
   if (bstr2.length() > 0)
   {
      ...
      memcpy(&bstr[bstr.get_size() - 1 - bstr2.length()], bstr2.data(), bstr2.length());
      bstr[bstr.get_size() - 1] = '\0';
...
bstring & bstring::operator += (const char * psz)
{
   ...
   if (psz != NULL)
   {
      ...
      memcpy(&m_puchData[get_size() - 1 - strlen(psz)], psz, strlen(psz));
      m_puchData[get_size() - 1] = '\0';
...
bstring & bstring::operator += (const string & str)
{
   ...
   memcpy(&m_puchData[get_size() - 1 - str.length()], str, str.length());
   m_puchData[get_size() - 1] = '\0';
...
char * bstring::GetBufferSetLength(strsize s)
{
...
void bstring::ReleaseBufferSetLength(strsize s)
{
...
const byte * bstring::data() const
{

   return (const byte *)get_data();
...
void bstring::assign(const bstring & bstr)
{
   ...
   memcpy(m_puchData, bstr.m_puchData, get_allocation_size());
