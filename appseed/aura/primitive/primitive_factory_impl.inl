...
void base_factory::creatable(int32_t iCount, bool bOverwrite, bool bAligned)
{
   ...
   ::id id = System.type_info <  T  >()->m_id;

   if (bOverwrite || !is_set(id))
   {

      factory_allocator * pallocator = get_allocator<T>(iCount, bAligned);

      factory_item_base * pitem = canew(creatable_factory_item<T>(get_app(), pallocator));
...
template < class T >
factory_allocator * base_factory::get_allocator(int32_t iCount, bool bAligned)
{

   sp(factory_allocator) & pallocator = m_mapAllocator[System.type_info <  T  > ()->m_id];

   if(pallocator.is_set())
      return pallocator.m_p;
