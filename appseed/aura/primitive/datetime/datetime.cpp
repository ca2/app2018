...
namespace datetime
{
   ...
   value span_strtotime(::aura::application * pbaseapp,::aura::str_context * pcontext,const char * pszSpanExpression)
   {
      ...
      for(int32_t i = 0; *psz; psz = ::str::utf8_inc(psz))
      {
         ...
         if(::str::ch::is_whitespace(psz))
         {
            ...
            if((bAdd || bMinus) && strNumber.has_char() && strText1.has_char())
            {
               ...
               {
                  _throw(simple_exception(get_app(), "now cannot be span"));
               }
               else
               {
                  _throw(not_implemented(pbaseapp));
               }
               strNumber.Empty();
               strText1.Empty();
               if(bAdd)
         ...
         else if(strChar == "+")
         {
            ...
            if(bAdd)
            {
               Sys(pbaseapp->m_psystem).log().trace("strtotime: invalid char +");
            }
            else if(bMinus)
            {
               Sys(pbaseapp->m_psystem).log().trace("strtotime: invalid char + on Minus state");
            }
         ...
         else if(strChar == "-")
         {
            ...
            if(bAdd)
            {
               Sys(pbaseapp->m_psystem).log().trace("strtotime: invalid char - on add state");
            }
            else if(bMinus)
            {
               Sys(pbaseapp->m_psystem).log().trace("strtotime: invalid char - on Minus state");
   ...
   value strtotime(::aura::application * pbaseapp,::aura::str_context * pcontext,const char * psz,int32_t & iPath,int32_t & iPathCount,bool bUTC)
   {
      ::datetime::time time;
      string str(psz);
      str.trim();
      ...
      if(str.get_length() >= 16)
      {
         ...
         {
            bBaseTime = true;
            Sys(pbaseapp->m_psystem).datetime().international().parse_str(str,set);
      ...
      if(!bBaseTime && cregexp_util::match(stra,str,"/^\\s*((\\d+)\\s*/\\s*(\\d+))((\\d|$)?!)/",true,4) > 0)
      {
         ...
         if(i1 != i2
               && i1 >= 1 && i1 <= 12
               && i2 >= 1 && i2 <=
               Sys(pbaseapp->m_psystem).datetime().get_month_day_count(time.GetYear(),i1))
         {
            bFirst = true;
            iCount++;
         }
         if(i2 >= 1 && i2 <= 12
               && i1 >= 1 && i1 <=
               Sys(pbaseapp->m_psystem).datetime().get_month_day_count(time.GetYear(),i2))
         {
            iCount++;
         }
         if(iCount > 0)
   ...
   string to_string(::aura::application * pbaseapp,::aura::str_context * pcontext,const ::datetime::value & value)
   {
      string str;
      if(value.m_bSpan)
      {
      ...
      else
      {
         ...
         if(time.GetSecond() == 0)
         {
            ...
            {
               str = time.Format("%Y-");
               Sys(pbaseapp->m_psystem).datetime().get_month_str(pcontext,time.GetMonth());
               str += time.Format("-%d");
            }
            else
            {
               str = time.Format("%Y-");
               str += Sys(pbaseapp->m_psystem).datetime().get_month_str(pcontext,time.GetMonth());
