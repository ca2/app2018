...
Serial::Serial (::aura::application * papp, const string &port, uint32_t baudrate, serial::Timeout timeout,
   ...
   : object(papp), pimpl_(canew(SerialImpl (papp, port, baudrate, bytesize, parity,
...
Serial::read (memory &buffer, size_t size)
{
   ScopedReadLock lock(this->pimpl_);
   memory bufferRead;
   bufferRead.allocate(size);
   size_t bytes_read = this->pimpl_->read (bufferRead.get_data(), bufferRead.get_size());
...
{
   ScopedWriteLock lock(this->pimpl_);
   return this->write_ (data.get_data(), data.size());
}
