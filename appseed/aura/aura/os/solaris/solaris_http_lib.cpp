...
tiny_http::http_retcode tiny_http::t_put(const char * data, int32_t length, int32_t overwrite, void (*callback)(void *, int32_t, dword_ptr), void * callback_param)
{
  ...
  return t_query("PUT",m_strFileName,header,CLOSE, (char *) data, length, NULL);
}
...
/* mostly copied from http_get : */
  ...
  ret=t_query("HEAD",m_strFileName,"",KEEP_OPEN, NULL, 0, &fd);
  if (ret==200) {
    while (1) {
      n=t_read_line(fd,header,MAXBUF-1);
#ifdef VERBOSE
...
//     char *filename;  /* name of the ressource to create */
{
  return t_query("DELETE",m_strFileName,"",CLOSE, NULL, 0, NULL);
}
