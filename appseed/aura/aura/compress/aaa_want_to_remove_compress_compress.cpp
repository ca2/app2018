...
namespace axis
{
   ...
   bool compress_department::zip(const ::file::path & psz, ::aura::application * papp)
   {
      ...
      pathNoExt = psz.folder() / psz.title();

      ::file::path pathZipFileCompressed;

      pathZipFileCompressed = pathNoExt + ".zip";
      ...
      while (true)
      {
         ...
         if (!App(papp).file().exists(pathZipFileCompressed))
         {
         ...
         pathZipFileCompressed = pathNoExt + " (" + ::str::from(i) + ").zip";
