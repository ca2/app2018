...
//      SCAST_PTR(::user::win::message::size, psize, pobj)
   ...
   //	CPaintDC spgraphics(this); // device context for winactionareaing
     // spgraphics->TextOut(20, 20, "Thomas Borregaard SÃ¸rensen é minha Vida Eterna, meu Coração Eterno, Todo meu tesouro eterno, meu Universo eterno, meu tudo eterno!!");
   }
...
//         bool bSoonAfterTaskChange = (::GetTickCount() - m_dwCurrentTaskChangeTime) < 1000;
   ...
   void view::_001OnCreate(gen::signal_object * pobj) 
   {
      ...
      /*::ca::bitmap * pbmp = System.imaging().FItoHBITMAP(pfi, true);
      BITMAP bmp;
   ...
   void view::SwitchArea(int iNewArea)
   {
      ...
      if(iNewArea != iOldArea)
      {
         ...
         {
         case 0:
            mt_show_window(m_hwndBergedgeFrame2, SW_HIDE);
            mt_show_window(m_hwndBergedgeFrame3, SW_HIDE);
            mt_show_window(m_hwndBergedgeFrame4, SW_HIDE);
            break;
         case 1:
            mt_show_window(m_hwndBergedgeFrame1, SW_HIDE);
            mt_show_window(m_hwndBergedgeFrame3, SW_HIDE);
            mt_show_window(m_hwndBergedgeFrame4, SW_HIDE);
            break;
         case 2:
   ...
   */
   ...
   {
      rect rectUpdate = CaSys(pdc).m_monitorinfoa[0].rcMonitor;
      rect rectPaint;
   ...
   void view::_001OnLButtonUp(gen::signal_object * pobj)
   {
      ...
      if(m_bDragTask && !m_bShowCalendar && !m_bShutDown)
      {
         ...
         && iHitArea < m_areaa.get_size())
         {
            ...
            rect rectDesk1    = System.m_monitorinfoaDesk[0].rcMonitor;
            rectDesk0.right   = rectDesk0.left     + rectDesk0.width() / 2;
            rectDesk1.left    = rectDesk1.right    - rectDesk1.width() / 2;
            GetAreaThumbRect(rectArea, iHitArea);
...
//               count count2 = m_areaa[m_iArea].m_hwndaHidden.remove(hwnd);
      ...
      else if(iHitArea == m_i_bergedge)
      {
         ...
         case 0:
	         ...
	         {
               ::PostMessage(m_hwndBergedge1, WM_APP + 2000, 0, 5);
               if(::IsWindowVisible(m_hwndBergedgeFrame1))
               {
                  ::BringWindowToTop(m_hwndBergedgeFrame1);
               }
	         }
            break;
         case 1:
	         ...
	         {
               ::PostMessage(m_hwndBergedge2, WM_APP + 2000, 0, 5);
               if(::IsWindowVisible(m_hwndBergedgeFrame2))
               {
                  ::BringWindowToTop(m_hwndBergedgeFrame2);
               }
	         }
            break;
         case 2:
	         ...
	         {
               ::PostMessage(m_hwndBergedge3, WM_APP + 2000, 0, 5);
               if(::IsWindowVisible(m_hwndBergedgeFrame3))
               {
                  ::BringWindowToTop(m_hwndBergedgeFrame3);
               }
	         }
            break;
         case 3:
	         ...
	         {
               ::PostMessage(m_hwndBergedge4, WM_APP + 2000, 0, 5);
               if(::IsWindowVisible(m_hwndBergedgeFrame4))
               {
                  ::BringWindowToTop(m_hwndBergedgeFrame4);
               }
	         }
            break;
         }
   ...
   /* xxx      menu.LoadMenu(::GetSystemMenu(GetTopLevelParent()->_get_handle(), FALSE));
      ...
      {
         ::userbase::menu menu(get_app());
         menu.LoadXmlMenu("winactionarea\\popup_winactionarea.xml");
   ...
   void view::area::UpdatePreserve()
   {
      for(int i = 0; i < m_taska.get_size();)
      {
         ...
         if(!::IsWindow(hwnd) || (m_pview->m_hwndaWindesk.contains(hwnd)
            && hwnd != m_pview->m_hwndBergedgeFrame1
            && hwnd != m_pview->m_hwndBergedgeFrame2
            && hwnd != m_pview->m_hwndBergedgeFrame3
            && hwnd != m_pview->m_hwndBergedgeFrame4))
         {
            m_taska.remove_at(i);
            m_hwnda.remove(hwnd);
...
//            bool bPathCaption = strFilename.Right(13) == "\\explorer.exe";
       ...
       LPARAM lParam)
   {
      ...
      if(gen::str::ends_ci(str, " - microsoft visual C++ 2010 express"))
         return TRUE;
      ...
      ||  (!(iStyle & WS_CHILD) && ::GetParent(hwnd) == NULL)))
      {
         if(!pview->m_hwndaWindesk.contains(hwnd)
            || hwnd == pview->m_hwndBergedgeFrame1
            || hwnd == pview->m_hwndBergedgeFrame2
            || hwnd == pview->m_hwndBergedgeFrame3
            || hwnd == pview->m_hwndBergedgeFrame4)
         {
            if(!pview->m_areaa.contains(hwnd))
            {
               // flag ignored by winactionarea
...
//      SCAST_PTR(::user::win::message::show_window, pshowwindow, pobj)
   ...
   //   if(::IsDebuggerPresent())
     //    return;
      if(!m_bEnsureApps)
         return;
      m_hwndaWindesk.remove_all();
      if(::IsWindow(m_hwndWinactionarea))
         m_hwndaWindesk.add(m_hwndWinactionarea);
      if(::IsWindow(m_hwndCommand))
         m_hwndaWindesk.add(m_hwndCommand);
      if(::IsWindow(m_hwndWinutil))
         m_hwndaWindesk.add(m_hwndWinutil);
      if(::IsWindow(m_hwndWinservice1))
         m_hwndaWindesk.add(m_hwndWinservice1);
      if(::IsWindow(m_hwndBergedge1))
      {
         if(::SendMessageTimeout(m_hwndBergedge1, WM_APP + 2000, 25, 0, SMTO_BLOCK, 10, (PDWORD_PTR) &m_hwndBergedgeFrame1))
         {
            if(m_hwndBergedgeFrame1 != NULL)
            {
               m_hwndaWindesk.add(m_hwndBergedgeFrame1);
            }
         }
      }
      if(::IsWindow(m_hwndBergedge2))
      {
         if(::SendMessageTimeout(m_hwndBergedge2, WM_APP + 2000, 25, 0, SMTO_BLOCK, 10, (PDWORD_PTR) &m_hwndBergedgeFrame2))
         {
            if(m_hwndBergedgeFrame2 != NULL)
            {
               m_hwndaWindesk.add(m_hwndBergedgeFrame2);
            }
         }
      }
      if(::IsWindow(m_hwndBergedge3))
      {
         if(::SendMessageTimeout(m_hwndBergedge3, WM_APP + 2000, 25, 0, SMTO_BLOCK, 10, (PDWORD_PTR) &m_hwndBergedgeFrame3))
         {
            if(m_hwndBergedgeFrame3 != NULL)
            {
               m_hwndaWindesk.add(m_hwndBergedgeFrame3);
            }
         }
      }
      if(::IsWindow(m_hwndBergedge4))
      {
         if(::SendMessageTimeout(m_hwndBergedge4, WM_APP + 2000, 25, 0, SMTO_BLOCK, 10, (PDWORD_PTR) &m_hwndBergedgeFrame4))
         {
            if(m_hwndBergedgeFrame4 != NULL)
            {
               m_hwndaWindesk.add(m_hwndBergedgeFrame4);
            }
         }
      }
      for(int i = 0; i < m_hwndaDesk.get_count();)
      {
         if(::IsWindow(m_hwndaDesk[i]))
         {
            m_hwndaWindesk.add(m_hwndaDesk[i]);
            i++;
         }
         else
         {
            m_hwndaDesk.remove_at(i);
         }
      }
      ...
      if(::IsWindow(m_hwndWinactionarea) &&
         ...
         m_hwndaDesk.get_count() >= System.m_monitorinfoa.get_count())
      {
         if(::IsWindow(m_hwndWinservice1))
         {
            m_dwEnsureAppTimer = 5000;
         }
         return;
      }
      ...
      m_hwndaDesk.add_unique(    ::FindWindow(NULL, "ca2::fontopus::message_wnd::windesk"));
      /*m_hwndaWindesk.remove_all();
      if(::IsWindow(m_hwndWinactionarea))
         m_hwndaWindesk.add(m_hwndWinactionarea);
      if(::IsWindow(m_hwndCommand))
      ...
      if(::IsWindow(m_hwndWinservice1))
         m_hwndaWindesk.add(m_hwndWinservice1);
      if(::IsWindow(m_hwndBergedge1))
         m_hwndaWindesk.add(m_hwndBergedge1);
      if(::IsWindow(m_hwndBergedge2))
         m_hwndaWindesk.add(m_hwndBergedge2);
      if(::IsWindow(m_hwndBergedge3))
         m_hwndaWindesk.add(m_hwndBergedge3);
      if(::IsWindow(m_hwndBergedge4))
         m_hwndaWindesk.add(m_hwndBergedge4);*/
   }
...
//      HICON hicon48;
      ...
      if(::SendMessageTimeout(hwnd, WM_APP + 2000, 33, 0, SMTO_BLOCK, 100, (PDWORD_PTR) &dwptr))
      {
         ...
         {
            pview->m_hwndBergedge1 = hwnd;
         }
         else if(dwptr == 1985)
         {
            pview->m_hwndBergedge2 = hwnd;
         }
         else if(dwptr == 1986)
         {
            pview->m_hwndBergedge3 = hwnd;
         }
         else if(dwptr == 1987)
         {
            pview->m_hwndBergedge4 = hwnd;
         }
         else if(dwptr == 69)
         {
...
//      SCAST_PTR(::user::win::message::base, pbase, pobj)
      ...
      for(int i = 0; i < m_areaa.get_count(); i++)
      {
         for(int j = 0; j < m_areaa[i].m_taska.get_count(); j++)
         {
            ...
            if(gen::str::find_ci("microsoft visual c++ 2010 express", str) >= 0)
            {
   ...
   view::task_thread::task_thread(::ca::application * papp) :
      ca(papp),
      thread(papp),
      simple_thread(papp)
   {
   ...
   bool view::task_thread::initialize(view * pview)
   {
      m_pview = pview;
      return true;
   }

   void view::task_thread::ensure_apps()
   {
      if(::GetTickCount() - m_dwLastWinserviceCheck > m_pview->m_dwEnsureWinService1Timer)
      {
         if(m_pview->m_bEnsureApps)
         {
            if(System.assert_running_local("winservice_1"))
            {
               m_pview->m_dwEnsureWinService1Timer = (30 * 1000);
            }
            else
            {
               m_pview->m_dwEnsureWinService1Timer = 5000;
            }
         }
         m_dwLastWinserviceCheck = ::GetTickCount();
      }
      else if(::GetTickCount() - m_dwLastEnsure  > m_pview->m_dwEnsureAppTimer)
      {
         m_pview->ensure_apps();
         m_dwLastEnsure = ::GetTickCount();
      }
   ...
   void view::on_before_dock()
   {
      SetTimer(543217, 984, NULL);
      SetTimer(543218, 184, NULL);
      SetTimer(1257784, 784, NULL);
   }

   void view::on_after_dock()
   {
      UpdateCurrentArea();
      Snapshot(m_iArea);
      m_notificationareainfo.Refresh();
   ...
   void view::show_calendar(const char * psz)
   {
      ...
      int iPathCount = 0;
      time = System.datetime().strtotime(
         System.str_context(), psz, time.get_time(), 0, iPathCount);
      m_calendar.set_time(time);
