      ...
      for(int32_t idx = 0; idx < hrdLocV->get_size(); idx++)
      {
         if (hrdLocV->element_at(idx).has_char())
         {
            ...
            try
            {
               ...
               ::file::byte_stream spfile(Application.file().get_file(strPath, ::file::mode_read | ::file::type_binary));
   ...
   TextHRDMapper *ParserFactory::createTextMapper(string nameID)
   {
      ...
      for(int32_t idx = 0; idx < hrdLocV->get_size(); idx++)
      {
         if (hrdLocV->element_at(idx).has_char())
         {
            try
            {
               ::file::byte_stream stream(Application.file().get_file(hrdLocV->element_at(idx), ::file::mode_read |::file::type_binary));
