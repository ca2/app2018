...
namespace win
{
   ...
   bool window::create_message_window(const char * pszName, ::ca::window_callback * pcallback)
   {
      ...
      else
      {
         ...
         rect(0, 0, 0, 0), System.window_from_os_data(HWND_MESSAGE), NULL))
         {
            return false;
         }
      }
   ...
   window::~window()
   {
      ...
      if(m_papp != NULL && m_papp->m_psystem != NULL && m_papp->m_psystem->m_pwindowmap != NULL)
      {
         m_papp->m_psystem->m_pwindowmap->m_map.remove_key((INT_PTR) get_handle());
      }
...
//      BOOL b;
   ...
   restart_mouse_hover_check:
         ...
         pbase->m_uiMessage == WM_CHAR)
      {
         ...
         ::user::interaction * puiFocus = dynamic_cast < ::user::interaction * > (System.get_keyboard_focus());
         if(puiFocus != NULL 
         && puiFocus->IsWindow()
...
*/
  ...
  /*  void CMDIFrameWnd::MDIActivate(::user::interaction* pWndActivate)
   ...
   CLASS_DECL_VMSWIN LRESULT AfxCallWndProc(::user::interaction * pinteraction, HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
   {
      ...
      {
         if(pinteraction->m_papp->on_run_exception((::ca::exception &) e))
            goto run;
         if(pinteraction->m_papp->final_handle_exception((::ca::exception &) e))
            goto run;
...
hwnd_map* PASCAL afxMapHWND(BOOL bCreate)
{
   ...
   return pState->m_pCurrentWinThread->m_pmapHWND;
}
   ...
   HCURSOR hCursor, HBRUSH hbrBackground, HICON hIcon)
{
   ...
   HINSTANCE hInst = ::win::get_thread()->m_papp->m_psystem->m_hInstance;
...
BOOL CLASS_DECL_VMSWIN AfxEndDeferRegisterClass(LONG fToRegisterParam, const char ** ppszClass)
{
   ...
   wndcls.lpfnWndProc = DefWindowProc;
   wndcls.hInstance = ::win::get_thread()->m_papp->m_psystem->m_hInstance;
