...
namespace win
{
   ...
   window::window()
   {
      m_pcallback = NULL;
      m_pbuffer = NULL;
      m_pwnd = NULL; // it may be assigned to this for example, when created
      ...
      ::user::interaction(papp)
   {
      ...
      m_pwnd = NULL; // it may be assigned to this for example, if created
      m_pwindowinterface = this;
      m_signalOnDraw.connect(this, &window::_002OnDraw);
      m_pguie = this;
	   set_handle(NULL);
   ...
   BOOL window::Attach(HWND hWndNew)
   {
      ...
      ::user::window_interface::m_hwnd = hWndNew;
   ...
   //   cs.hMenu = hWndParent == NULL ? NULL : nIDorHMenu;
      cs.hMenu = NULL;
	   cs.hInstance = Application.m_hInstance;
      ...
      else
      {
	   ...
	   if (hWnd == NULL)
	   {
         ...
         TRACE(traceAppMsg, 0, "Warning: Window creation failed: GetLastError returned:\n");
         TRACE(traceAppMsg, 0, "%s\n", strMessage);
         if(dwLastError == 0x0000057e)
         {
            Application.simple_message_box(NULL, "Cannot create a top-level child window.");
         }
         else
         {
      ...
      WNDCLASS wndcls;
		if(lpszClassName != NULL &&
			GetClassInfo(Application.m_hInstance, lpszClassName, &wndcls) &&
			wndcls.hIcon != NULL)
      {
   ...
   bool window::create_message_window(const char * pszName, ::ca::window_callback * pcallback)
   {
      ...
      else
      {
         ...
         if(!create(NULL, pszName, 0, 
         rect(0, 0, 0, 0), Application.window_from_os_data(HWND_MESSAGE), NULL))
         {
            return false;
         }
      }
   ...
   window::~window()
   {
      if(::IsWindow(get_handle()))
      {
         if(GetParent() != NULL)
         {
            ...
            GetParent()->RemoveChild(m_pguie);
            if(GetParent()->m_pguie != NULL)
            {
               GetParent()->m_pguie->RemoveChild(this);
               GetParent()->m_pguie->RemoveChild(m_pguie);
            }
            if(GetParent()->m_pimpl != NULL)
            {
               GetParent()->m_pimpl->RemoveChild(this);
               GetParent()->m_pimpl->RemoveChild(m_pguie);
            }
         }
      }
      if(GetOwner() != NULL)
      {
         ...
         GetOwner()->RemoveChild(m_pguie);
         GetOwner()->RemoveChild(m_pimpl);
         if(GetOwner()->m_pguie != NULL)
         {
            GetOwner()->m_pguie->RemoveChild(this);
            GetOwner()->m_pguie->RemoveChild(m_pguie);
            GetOwner()->m_pguie->RemoveChild(m_pimpl);
         }
         if(GetOwner()->m_pimpl != NULL)
         {
            GetOwner()->m_pimpl->RemoveChild(this);
            GetOwner()->m_pimpl->RemoveChild(m_pguie);
            GetOwner()->m_pimpl->RemoveChild(m_pimpl);
         }
      }
      for(int i = 0; i < m_uiptra.get_size(); i++)
      {
         ::user::interaction * puie = m_uiptra.derived_at(i);
         if(puie->m_pwnd == this)
         {
            puie->m_pwnd = NULL;
         }
      ...
      if(m_pguie != NULL)
      {
         for(int i = 0; i < m_pguie->m_uiptra.get_size(); i++)
         {
            ::user::interaction * puie = m_pguie->m_uiptra.derived_at(i);
            if(puie->m_pwnd == this)
            {
               puie->m_pwnd = NULL;
            }
      ...
      if(m_pfont != NULL)
      {
	   ...
	   if (get_handle() != NULL)
	   {
		   TRACE(traceAppMsg, 0, "Warning: calling DestroyWindow in window::~window; "
   ...
   void window::_001OnNcDestroy(gen::signal_object * pobj)
   {
      ...
      if(GetParent() != NULL)
      {
         GetParent()->RemoveChild(this);
         GetParent()->RemoveChild(m_pguie);
	      GetParent()->RemoveChild(m_pimpl);
         if(GetParent()->m_pguie != NULL)
         {
            GetParent()->m_pguie->RemoveChild(this);
            GetParent()->m_pguie->RemoveChild(m_pguie);
		      GetParent()->m_pguie->RemoveChild(m_pimpl);
         }
         if(GetParent()->m_pimpl != NULL)
         {
            GetParent()->m_pimpl->RemoveChild(this);
            GetParent()->m_pimpl->RemoveChild(m_pguie);
		      GetParent()->m_pimpl->RemoveChild(m_pimpl);
         }
      }
      if(GetOwner() != NULL)
      {
      ...
      ::ca::thread* pThread = Application.GetThread();
	   if (pThread != NULL)
	   {
...
//		   base_object* p=NULL;
   ...
   #ifdef _DEBUG
			   ASSERT(pMap->lookup_permanent(hWndOrig) == NULL);
   ...
   {
	   ASSERT(::IsWindow(get_handle()));
	   rString = "";    // empty without deallocating
   ...
   { ASSERT(::IsWindow((HWND)get_os_data())); return ::GetWindowInfo((HWND)get_os_data(), pwi); }
...
/*	   application* pApp = &Application;
	   ...
	   TRACE(traceAppMsg, 0, "WinHelp: pszHelpFile = '%s', dwData: $%lx, fuCommand: %d.\n", pApp->m_pszHelpFilePath, dwData, nCmd);

	   // finally, run the Windows Help engine
   /* trans	if (!::WinHelp(WIN_WINDOW(pWnd)->get_handle(), pApp->m_pszHelpFilePath, nCmd, dwData))
	   {
		   // linux Application.simple_message_box(AFX_IDP_FAILED_TO_LAUNCH_HELP);
         Application.simple_message_box("Failed to launch help");
	   }*/
   }
   ...
   //{
     // throw not_implemented_exception();
      /*
	   ...
	   TRACE(traceAppMsg, 0, "HtmlHelp: pszHelpFile = '%s', dwData: $%lx, fuCommand: %d.\n", pApp->m_pszHelpFilePath, dwData, nCmd);
...
//      BOOL b;
   ...
   void window::message_handler(gen::signal_object * pobj)
   {
      ...
      {
         Application.GetThread()->step_timer();
      }
         ...
         pbase->m_uiMessage == WM_MOUSEMOVE)
      {
         ...
         if(m_bOSNativeMouseMessagePosition)
         {
            ...
            ::GetWindowRect(GetSafeHwnd(), &rectWindow32);
         ...
         if(!pmouse->m_bTranslated)
         {
            ...
            if(Application.m_monitorinfoa.get_count() > 0)
            {
               if(rectWindow.left >= Application.m_monitorinfoa[0].rcMonitor.left)
                  pmouse->m_pt.x += (LONG) rectWindow.left;
               if(rectWindow.top >= Application.m_monitorinfoa[0].rcMonitor.top)
   ...
   restart_mouse_hover_check:
         ...
         if(!m_bMouseHover)
         {
            string strApp = App(::get_app()).m_strAppName;
            if(strApp == "netshareserver")
            {
               ...
               //Application.TrackMouseEvent(&tme);
            }
            else
            {
               m_bMouseHover = true;
         ...
         pbase->m_uiMessage == WM_CHAR)
      {
         ...
         if(Application.m_pkeyboardfocus != NULL && Application.m_pkeyboardfocus->IsWindow()
            && Application.m_pkeyboardfocus->GetTopLevelParent() != NULL &&
            (Application.m_pkeyboardfocus->GetTopLevelParent() == Application.m_puiInitialPlaceHolderContainer
            || Application.m_pkeyboardfocus->GetTopLevelParent()->m_pwnd->IsWindowVisible()))
         {
            Application.m_pkeyboardfocus->SendMessage(pkey);
            if(pbase->m_bRet)
               return;
         }
         else if(!pkey->m_bRet)
...
//	   int nCode = pNMHDR->code;
   ...
   ::user::interaction * PASCAL window::GetDescendantWindow(::user::interaction * hWnd, id id)
   {
      ...
      for(int i = 0; i < hWnd->m_uiptra.get_count(); i++)
      {
         if(hWnd->m_uiptra.derived_at(i)->GetDlgCtrlId() == id)
         {
            if(hWnd->m_uiptra.derived_at(i)->GetDescendantWindow(id))
               return hWnd->m_uiptra.derived_at(i)->GetDescendantWindow(id);
            else
               return hWnd->m_uiptra.derived_at(i);
         }
      }
   ...
   */
	      ...
	      UINT nFlags, LPRECT lpRectParam, LPCRECT lpRectClient, BOOL bStretch)
   {
      ...
      if(m_pguie != this && m_pguie != NULL)
      {
	      ...
	      for (int i = 0; i < m_pguie->m_uiptra.get_count();	i++)
	      {
            ::user::interaction * hWndChild = m_pguie->m_uiptra.derived_at(i);
		      id id = hWndChild->GetDlgCtrlId();
		      ::user::interaction * pWnd = hWndChild;
		      if (id == (int) nIdLeftOver)
			      hWndLeftOver = hWndChild;
      ...
      else
      {
	      ...
	      for (int i = 0; i < m_uiptra.get_count();	i++)
	      {
            ::user::interaction * hWndChild = m_uiptra.derived_at(i);
		      id id = hWndChild->GetDlgCtrlId();
		      ::user::interaction * pWnd = hWndChild;
		      if (id == (int) nIdLeftOver)
			      hWndLeftOver = hWndChild;
   ...
   BOOL window::HandleFloatingSysCommand(UINT nID, LPARAM lParam)
   {
	   ...
	   case SC_CLOSE:
	   case SC_KEYMENU:
		   // Check lParam.  If it is 0L, then the ::user::profile may have done
...
/*	   application* pApp = &Application;
   ...
   BOOL window::OnHelpInfo(HELPINFO* /*pHelpInfo*/)
   {
	   if (!(GetStyle() & WS_CHILD))
	   {
		   ::user::interaction* pMainWnd = Application.GetMainWnd();
		   if (pMainWnd != NULL &&
   ...
   LRESULT window::OnDisplayChange(WPARAM, LPARAM)
   {
	   ...
	   if (Application.GetMainWnd() == this)
	   {
...
//      int iCount = wndaApp.get_count();
   ...
   /*         else if(::GetWindowLong(wndaApp[j], GWL_EXSTYLE) & WS_EX_LAYERED)
         ...
         {
            if(!::IsWindowVisible(wndaApp[j]) || ::IsIconic(wndaApp[j]))
               continue;
   ...
   {
      UNREFERENCED_PARAMETER(pobj);
      App(::get_app()).get_event(m_pthread)->SetEvent();
      CSingleLock sl(App(::get_app()).get_event(m_papp->s_ptwf));
      sl.Lock(8400);
   }
...
//      DWORD dw = ::GetLastError();
	   ...
	   HBRUSH hbrGray, COLORREF clrText)
   {
	   if (hDC == NULL)
	   {
		   ...
		   TRACE(traceAppMsg, 0, "Warning: hDC is NULL in window::GrayCtlColor; WM_CTLCOLOR not processed.\n");
		   return FALSE;
	   }
   ...
   /*BOOL window::UpdateData(BOOL bSaveAndValidate)
   {
	   ...
	   {
		   // validation failed - ::user::profile already alerted, fall through
		   ASSERT(!bOK);											
   ...
   void window::CenterWindow(::user::interaction * pAlternateOwner)
   {
	   ...
	   if (!(dwStyle & WS_CHILD))
	   {
		   ...
		   if (hWndCenter == NULL)
		   {
			   HWND hwDefault = Application.GetMainWnd()->_get_handle();
   ...
   {
	   ASSERT(::IsWindow(get_handle())); // window must be created
...
ExitModal:
   ...
   BOOL window::SubclassWindow(HWND hWnd)
   {
      ...
      m_pfnSuper = (WNDPROC)::GetWindowLongPtr(hWnd, GWLP_WNDPROC);
	   ...
	   {
		   TRACE(traceAppMsg, 0, "Error: Trying to use SubclassWindow with incorrect window\n");
...
*/
   ...
   bool window::SetWindowPos(int z, int x, int y, int cx, int cy, UINT nFlags)
   {
      ...
      */
      ASSERT(::IsWindow(get_handle())); 
   /*   return ::SetWindowPos(get_handle(), pWndInsertAfter->GetSafeHwnd(),
      ...
      if(GetExStyle() & WS_EX_LAYERED)
      {
         ...
         {
            ::SetWindowPos(GetSafeHwnd(), (HWND) z, x, y, cx, cy, nFlags);
            ShowWindow(SW_SHOW);
         }
         else
         {
         ...
         if(m_papp != NULL && m_papp->s_ptwf != NULL)
         {
            m_papp->s_ptwf->synch_redraw();
         }
      }
      else
      {
...
/*   guie_message_wnd::guie_message_wnd(::ca::application * papp) :
   ...
   bool window::IsZoomed()
   {
      ...
      {
         return ::IsZoomed(get_handle()) != FALSE;
      }
   }
   ...
   ::user::interaction * window::get_capture()
   {
      ...
      if(hwndCapture == get_handle())
      {
         ...
         else
         {
            if(m_pguie != NULL)
            {
               if(m_pguie->m_pwnd != NULL && WIN_WINDOW(m_pguie->m_pwnd)->m_pguieCapture != NULL)
               {
                  return WIN_WINDOW(m_pguie->m_pwnd)->m_pguieCapture;
               }
               else
               {
                  return m_pguie;
   ...
   { return ((HWND) WIN_WINDOW(const_cast < ::ca::window * > (&wnd))->GetSafeHwnd()) == get_handle(); }
   BOOL window::operator!=(const ::ca::window& wnd) const
	   { return ((HWND) WIN_WINDOW(const_cast < ::ca::window * > (&wnd))->GetSafeHwnd()) != get_handle(); }
    HWND window::GetSafeHwnd() const
	   { return this == NULL ? NULL : get_handle(); }
    DWORD window::GetStyle()
   { ASSERT(::IsWindow(get_handle())); return (DWORD)::GetWindowLong(get_handle(), GWL_STYLE); }
    DWORD window::GetExStyle()
   { ASSERT(::IsWindow(get_handle())); return (DWORD)::GetWindowLong(get_handle(), GWL_EXSTYLE); }
    BOOL window::ModifyStyle(DWORD dwRemove, DWORD dwAdd, UINT nFlags)
	   { ASSERT(::IsWindow(get_handle())); return ModifyStyle(get_handle(), dwRemove, dwAdd, nFlags); }
    BOOL window::ModifyStyleEx(DWORD dwRemove, DWORD dwAdd, UINT nFlags)
	   { ASSERT(::IsWindow(get_handle())); return ModifyStyleEx(get_handle(), dwRemove, dwAdd, nFlags); }
   ...
   #pragma pop_macro("SendMessage")
    BOOL window::PostMessage(UINT message, WPARAM wParam, LPARAM lParam)
	   { ASSERT(::IsWindow(get_handle())); return ::PostMessage(get_handle(), message, wParam, lParam); }
    BOOL window::DragDetect(POINT pt) const
	   { ASSERT(::IsWindow(get_handle())); return ::DragDetect(get_handle(), pt); }
   ...
   int window::GetWindowText(__out_ecount_part_z(nMaxCount, return + 1) LPTSTR lpszString, __in int nMaxCount)
    ...
    int window::GetWindowTextLength()
	   { ASSERT(::IsWindow(get_handle())); return ::GetWindowTextLength(get_handle()); }
    ...
    ::ca::font* window::GetFont()
	   { ASSERT(::IsWindow(get_handle())); return m_pfont; }
    void window::DragAcceptFiles(BOOL bAccept)
	   { ASSERT(::IsWindow(get_handle())); ::DragAcceptFiles(get_handle(), bAccept); }
    ...
    ::user::interaction* window::EnsureTopLevelParent()
   {
    ...
    void window::BringWindowToTop()
	   { ASSERT(::IsWindow(get_handle())); ::BringWindowToTop(get_handle()); }
    void window::MapWindowPoints(::ca::window * pwndTo, LPPOINT lpPoint, UINT nCount)
	   { ASSERT(::IsWindow(get_handle())); ::MapWindowPoints(get_handle(), pwndTo->GetSafeHwnd(), lpPoint, nCount); }
    void window::MapWindowPoints(::ca::window * pwndTo, LPRECT lpRect)
	   { ASSERT(::IsWindow(get_handle())); ::MapWindowPoints(get_handle(), pwndTo->GetSafeHwnd(), (LPPOINT)lpRect, 2); }
    ...
    ::ca::graphics * window::GetWindowDC()
	   { ASSERT(::IsWindow(get_handle())); return ::win::graphics::from_handle(::GetWindowDC(get_handle())); }
	   ...
	   { 
                  if(get_handle() == NULL)
         {
             return ::ReleaseDC(NULL, (dynamic_cast<::win::graphics * >(pgraphics))->get_handle1()); 
                  ...
                  {
                     ASSERT(::IsWindow(get_handle())); return ::ReleaseDC(get_handle(), (dynamic_cast<::win::graphics * >(pgraphics))->get_handle1()); 
                  }
    }
    void window::UpdateWindow()
	   { ASSERT(::IsWindow(get_handle())); ::UpdateWindow(get_handle()); }
    void window::SetRedraw(BOOL bRedraw)
	   { ASSERT(::IsWindow(get_handle())); ::SendMessage(get_handle(), WM_SETREDRAW, bRedraw, 0); }
    BOOL window::GetUpdateRect(LPRECT lpRect, BOOL bErase)
	   { ASSERT(::IsWindow(get_handle())); return ::GetUpdateRect(get_handle(), lpRect, bErase); }
    int window::GetUpdateRgn(::ca::rgn* pRgn, BOOL bErase)
	   { ASSERT(::IsWindow(get_handle())); return ::GetUpdateRgn(get_handle(), (HRGN)pRgn->get_os_data(), bErase); }
    void window::Invalidate(BOOL bErase)
	   { ASSERT(::IsWindow(get_handle())); ::InvalidateRect(get_handle(), NULL, bErase); }
    void window::InvalidateRect(LPCRECT lpRect, BOOL bErase)
	   { ASSERT(::IsWindow(get_handle())); ::InvalidateRect(get_handle(), lpRect, bErase); }
    void window::InvalidateRgn(::ca::rgn* pRgn, BOOL bErase)
	   { ASSERT(::IsWindow(get_handle())); ::InvalidateRgn(get_handle(), (HRGN)pRgn->get_os_data(), bErase); }
    ...
    BOOL window::IsWindowVisible()
	   { ASSERT(::IsWindow(get_handle())); return ::IsWindowVisible(get_handle()); }
    void window::ShowOwnedPopups(BOOL bShow)
	   { ASSERT(::IsWindow(get_handle())); ::ShowOwnedPopups(get_handle(), bShow); }
    void window::SendMessageToDescendants(
	   UINT message, WPARAM wParam, LPARAM lParam, BOOL bDeep, BOOL bOnlyPerm)
	   { 
         
         ASSERT(::IsWindow(get_handle())); window::SendMessageToDescendants(get_handle(), message, wParam, lParam, bDeep,
		   bOnlyPerm); 
      ...
      for(int i = 0; i < m_pguie->m_uiptra.get_count(); i++)
      {
         try
         {
			   m_pguie->m_uiptra.derived_at(i)->SendMessage(message, wParam, lParam);
         }
         catch(...)
         {
         }
		   if (bDeep)
		   {
            ...
            try
            {
			      m_pguie->m_uiptra.derived_at(i)->SendMessageToDescendants(message, wParam, lParam, bDeep, bOnlyPerm);
            }
            catch(...)
            {
            }
    ...
    ::user::interaction * window::GetDescendantWindow(id id)
	   { ASSERT(::IsWindow(get_handle())); return window::GetDescendantWindow(this, id); }
    ...
    { 
       if(m_papp->s_ptwf->m_bProDevianMode)
          return TRUE;
       ASSERT(::IsWindow(get_handle())); 
...
int window::GetCheckedRadioButton(int nIDFirstButton, int nIDLastButton)
{
    ...
    ::ca::window * window::GetNextDlgGroupItem(::ca::window * pWndCtl, BOOL bPrevious) const
	   { ASSERT(::IsWindow(get_handle())); return ::win::window::from_handle(::GetNextDlgGroupItem(get_handle(),
			   pWndCtl->GetSafeHwnd(), bPrevious)); }
    ::ca::window * window::GetNextDlgTabItem(::ca::window * pWndCtl, BOOL bPrevious) const
	   { ASSERT(::IsWindow(get_handle())); return ::win::window::from_handle(::GetNextDlgTabItem(get_handle(),
			   pWndCtl->GetSafeHwnd(), bPrevious)); }
    ...
    ::ca::window * window::SetParent(::ca::window * pWndNewParent)
	   { ASSERT(::IsWindow(get_handle())); return ::win::window::from_handle(::SetParent(get_handle(),
   ...
   #pragma pop_macro("MessageBox")
    ...
    void window::HideCaret()
	   { ASSERT(::IsWindow(get_handle())); ::HideCaret(get_handle()); }
    void window::ShowCaret()
	   { ASSERT(::IsWindow(get_handle())); ::ShowCaret(get_handle()); }
    BOOL window::SetForegroundWindow()
	   { ASSERT(::IsWindow(get_handle())); return ::SetForegroundWindow(get_handle()); }
    ...
    BOOL window::SendNotifyMessage(UINT message, WPARAM wParam, LPARAM lParam)
	   { ASSERT(::IsWindow(get_handle())); return ::SendNotifyMessage(get_handle(), message, wParam, lParam); }

   // Win4
    HICON window::SetIcon(HICON hIcon, BOOL bBigIcon)
	   { ASSERT(::IsWindow(get_handle())); return (HICON)SendMessage(WM_SETICON, bBigIcon, (LPARAM)hIcon); }
   ...
   // Default message map implementations
    ...
    {
       SCAST_PTR(::user::win::message::base, pbase, pobj);
       if(Application.get_cursor() != NULL
          && Application.get_cursor()->m_ecursor != ::visual::cursor_system)
       {
         ::SetCursor(NULL);
       }
       pbase->set_lresult(1);
  ...
  /*  void CMDIFrameWnd::MDIActivate(::user::interaction* pWndActivate)
   ...
   /* trans		frame_window* pFrame = command_target::GetRoutingFrame_();
		   if (pFrame != NULL)
			   ...
			   hWnd = Application.GetMainWnd()->GetSafeHwnd();*/
	   }
   ...
   void window::OnPointerClear(::user::interaction * pui)
   {
      m_guieptraMouseHover.remove(pui);
      ::ca::window::OnPointerClear(pui);
   }
   ...
   CLASS_DECL_VMSWIN LRESULT AfxCallWndProc(::user::interaction * pinteraction, HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
   {
      ...
      // in debug builds and warn the ::user::profile.
      ::ca::smart_pointer < ::user::win::message::base > spbase(pinteraction->get_base(hWnd, nMsg, wParam, lParam));
      ...
      {
         if(get_app()->on_run_exception((::ca::exception &) e))
            goto run;
         if(get_app()->final_handle_exception((::ca::exception &) e))
            goto run;
      ...
      catch(base_exception * pe)
      {
	      ...
	      TRACE(traceAppMsg, 0, "Warning: Uncaught exception in message_handler (returning %ld).\n", spbase->get_lresult());
	      pe->Delete();
      }
   run:
   ...
   _AfxCbtFilterHook(int code, WPARAM wParam, LPARAM lParam)
   {
      ...
      if (pWndInit != NULL || (!(lpcs->style & WS_CHILD) && !bContextIsDLL))
      {
	      ...
	      if (pWndInit != NULL)
	      {
            ...
            pWndInit->m_pthread = Application.GetThread();
...
#endif
	   pState->m_pmapHWND = new hwnd_map;
   ...
   HCURSOR hCursor, HBRUSH hbrBackground, HICON hIcon)
{
   ...
   HINSTANCE hInst = Application.m_hInstance;
...
_AfxHandleActivate(::ca::window * pWnd, WPARAM nState, ::ca::window * pWndOther)
{
   ...
   if (!(WIN_WINDOW(pWnd)->GetStyle() & WS_CHILD))
   {
	   ...
	   if (pTopLevel && (pWndOther == NULL || !::IsWindow(WIN_WINDOW(pWndOther)->get_handle()) || pTopLevel != WIN_WINDOW(pWndOther)->GetTopLevelParent()))
	   {
         ...
         {
            hWnd2[1] = WIN_WINDOW(pWndOther)->GetSafeHwnd();
         }
		   // send it...
...
BOOL CLASS_DECL_VMSWIN AfxEndDeferRegisterClass(LONG fToRegisterParam, const char ** ppszClass)
{
   ...
   wndcls.lpfnWndProc = DefWindowProc;
   wndcls.hInstance = Application.m_hInstance;
