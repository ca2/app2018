   ...
   //   cs.hMenu = hWndParent == NULL ? NULL : nIDorHMenu;
      ...
      else
      {
	   ...
	   if (hWnd == NULL)
	   {
         ...
         {
            Application.simple_message_box("Cannot create a top-level child window.");
         }
         else
         {
            Application.simple_message_box(strMessage);
         }
	   }
   #endif
...
//		   base_object* p=NULL;
   ...
   void window::Dump(CDumpContext & dumpcontext) const
   {
...
//      BOOL b;
   ...
   restart_mouse_hover_check:
         ...
         && m_pguieCapture != this
         && m_pguieCapture->m_pimpl != this
         && m_pguieCapture->m_pguie != this)
         {
...
//      int iCount = wndaApp.get_count();
   ...
   {
      UNREFERENCED_PARAMETER(pobj);
      Application.get_event(m_pthread)->SetEvent();
      CSingleLock sl(Application.get_event(m_papp->s_ptwf));
      sl.Lock(8400);
   }
